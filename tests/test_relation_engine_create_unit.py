...
def people_string_id_parquet(tmp_path):
    ...
    pd.DataFrame({"id": ["a1", "a2"], "name": ["Alice", "Bob"]}).to_parquet(
        path
    )
    return path
...
def _streaming_people_no_id_ctx(people_no_id_parquet) -> Context:
    ctx = _streaming_ctx()
    register_streaming_source(
        ctx, "Person", data_source_from_uri(str(people_no_id_parquet))
    )
    return ctx
...
def _streaming_people_string_id_ctx(people_string_id_parquet) -> Context:
    ...
    register_streaming_source(
        ctx,
        "Person",
        data_source_from_uri(str(people_string_id_parquet)),
        id_col="id",
    )
    return ctx
...
class TestEligibility:
    ...
    def test_ineligible_non_integer_id_col(
        self, people_string_id_parquet
    ) -> None:
...
class TestExecution:
    ...
    def test_sequence_auto_increments_across_multiple_creates(
        self, people_parquet
    ) -> None:
        ctx = _streaming_people_ctx(people_parquet)
    ...
    def test_execute_relation_create_without_id_col(
        self, people_no_id_parquet
    ) -> None:
        ctx = _streaming_people_no_id_ctx(people_no_id_parquet)
        query = _ast("CREATE (p:Person {name: 'Dave', age: 40})")
        assert is_relation_create_eligible(query, ctx)
