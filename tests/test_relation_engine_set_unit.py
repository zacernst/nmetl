...
class TestExecution:
    def test_execute_relation_set_updates_matching_rows_only(
        self, people_parquet
    ) -> None:
        ctx = _streaming_people_ctx(people_parquet)
        query = _ast(
...
class TestNewColumn:
    ...
    def test_eligible_new_target_property(self, people_parquet) -> None:
        ctx = _streaming_people_ctx(people_parquet)
        assert is_relation_set_eligible(
            _ast("MATCH (p:Person) WHERE p.age > 28 SET p.tier = 'senior'"),
            ctx,
        )
    ...
    def test_execute_creates_column_and_leaves_unmatched_rows_null(
        ...
        ctx = _streaming_people_ctx(people_parquet)
        query = _ast("MATCH (p:Person) WHERE p.age > 28 SET p.tier = 'senior'")
        assert is_relation_set_eligible(query, ctx)
        execute_relation_set(query, ctx)

        got = ctx.backend.connection.execute(
            'SELECT name, tier FROM "_streaming_source_Person" ORDER BY name',
        ).fetchdf()
        rows = {
            name: _na_to_none(v) for name, v in zip(got["name"], got["tier"])
        }
        assert rows == {"Alice": "senior", "Bob": None, "Carol": "senior"}

    def test_second_query_can_read_the_new_column_immediately(
        self, people_parquet
    ) -> None:
        # The registry's attr_map is mutated in place, so a later query in
        # the same run sees the new property without rebuilding the Context.
        ctx = _streaming_people_ctx(people_parquet)
        query = _ast("MATCH (p:Person) WHERE p.age > 28 SET p.tier = 'senior'")
        execute_relation_set(query, ctx)
        out = Star(context=ctx).execute_query(
            "MATCH (p:Person) RETURN p.name AS name, p.tier AS tier",
        )
        rows = {
            name: _na_to_none(v) for name, v in zip(out["name"], out["tier"])
        }
        assert rows["Alice"] == "senior"
