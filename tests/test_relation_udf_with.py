    ...
    def test_unregistered_udf_ineligible(self) -> None:
        assert not is_relation_eligible(
            ASTConverter.from_cypher(
                "MATCH (n:Person) WITH mystery(n.name) AS s RETURN s AS out"
            ),
            _ctx(),
        )
...
class TestValues:
    ...
    def test_udf_inside_aggregate_count(self) -> None:
        ...
        out = _run(
            "MATCH (n:Person) WITH count(plus_ten(n.age)) AS c RETURN c AS cnt"
        )
...
class TestCountScalar:
    def test_count_scalar_counts_non_null(self) -> None:
        ...
        out = _run(
            "MATCH (n:Person) WITH n.age AS age WITH count(age) AS c RETURN c AS cnt"
        )
        assert out["cnt"].iloc[0] == 3
