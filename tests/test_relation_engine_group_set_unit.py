"""Phase 2 first slice (docs/fastopendata_streaming_qualification_plan.md) —
...
with a second required MATCH, multiple aggregates combined into a single
SET expression, ``WITH DISTINCT``) is deferred to Phase 2b and simply falls
back to the pandas engine, covered here by the ``TestEligibility`` negative
cases.
"""
...
class TestEligibility:
    ...
    def test_ineligible_set_expression_combines_aliases(
        self, household_parquet, puma_parquet, located_in_parquet
    ) -> None:
        ctx = _streamed_context(
            household_parquet, puma_parquet, located_in_parquet
    ...
    def test_ineligible_no_relationship_in_pattern(self, puma_parquet) -> None:
        ctx = _streaming_ctx()
        register_streaming_source(
            ctx,
            "PUMA",
    ...
    def test_ineligible_group_var_has_no_streaming_source(
