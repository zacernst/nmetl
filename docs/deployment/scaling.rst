...
.. code-block:: python
   ...
   # Select a backend programmatically (hint is keyword-only)
...
.. list-table::
   ...
   * - Pandas
     ...
     - Minimum backend every ``Context`` resolves to
   * - DuckDB
     - Production-ready
     - Large CSV/Parquet scans, analytical joins/aggregations
     - Real dispatch via ``context.backend`` -- not just data loading
   * - Polars
     - Experimental
     - CPU-bound transforms
     - Passes the equivalence suite but round-trips pandas<->polars per
       op; not yet in the e2e acceptance matrix
   * - Spark
     - Implemented, explicit-only
     - Distributed joins/aggregations on large datasets
     - All 16 ``BackendEngine`` methods implemented; excluded from the
       ``auto`` fallback chain (JVM startup cost) -- must be requested
       explicitly (``backend="spark"``)

.. note::

   Backend selection is a first-class, user-facing choice, not an internal
   implementation detail: pass ``backend=`` to ``ContextBuilder.build()`` or
   ``Context()`` directly (``Context(..., backend="duckdb")``), or set
   ``backend_engine`` in a pipeline's YAML config
   (``PipelineConfig.backend_engine``).  ``ContextBuilder.from_dict()`` is
   the one exception -- it does not accept a ``backend`` parameter.
...
The Docker Compose setup includes an optional Spark cluster.  It's the same
cluster the ``spark`` backend engine connects to (via ``SPARK_MASTER_URL``)
when a query or pipeline explicitly requests ``backend="spark"`` -- not just
infrastructure for external data processing.  To scale:
