...
Engine tests (pycypher and shared) live in ``tests/`` at the repository
root; each other package keeps its own suite under ``packages/<name>/tests``
(``packages/nmetl/tests`` for the CLI, pipeline config, sinks, and REPL).
All of them are collected by a bare ``pytest`` via ``testpaths``.  The root
suite uses a flat directory structure — each test file targets a specific
...
``neo4j`` service and no ``make test-neo4j`` target.  Tests that exercise
``Neo4jSink`` are marked with the ``neo4j`` pytest marker and require you to
point them at your own Neo4j instance:

.. code-block:: bash

   # Point at your own Neo4j instance
   export NEO4J_URI=bolt://localhost:7687
   export NEO4J_PASSWORD=secret

   # Run just the Neo4j-marked tests
   uv run pytest -m neo4j

   # The full suite runs these too, but they self-skip via a fixture
   # (neo4j_driver) when no live instance is reachable -- CI does not
   # pass -m "not neo4j"
