...
This is a **monorepo workspace** containing several interdependent packages:
...
.. code-block:: text
   ...
   │   ├── pycypher/       # Cypher parser, AST, execution engine, ingestion layer
   │   └── nmetl/          # ETL layer: pipeline YAML config, validation, `nmetl` CLI
   ├── tests/              # Engine (pycypher + shared) test suite
   ├── docs/               # Documentation
...
- **pycypher** depends on **shared** only; it never imports nmetl
