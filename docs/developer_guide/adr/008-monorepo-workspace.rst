...
.. code-block:: text
   ...
   └── nmetl/          # ETL layer and CLI (depends on shared + pycypher)
...
- Dependency order (``shared`` → ``pycypher`` → ``nmetl``) prevents
...
- Users can install ``pycypher`` alone without ``nmetl`` dependencies.
