...
def demo_data_preview():
    ...
    csv_dialog = DataPreviewDialog(
        source_uri=str(files["customers.csv"]), source_id="customers"
    )
    print(f"\nCSV dialog created: {csv_dialog.dialog_title}")

    json_dialog = DataPreviewDialog(
        source_uri=str(files["orders.json"]), source_id="orders"
    )
    ...
    print(
        "4. Use Tab to switch between Sample Data, Schema, and Statistics tabs"
    )
