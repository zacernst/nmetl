    ...
    *,
    ...
    if show_complexity:
        ...
        if not hotspots:
            ...
            print(
                f"  {len(errors)} ERROR (>={COMPLEXITY_ERROR}), {len(warns)} WARN (>={COMPLEXITY_WARN})"
            )
            print()
    ...
    if show_lint:
        ...
        print(
            f"  Missing annotations in production code: {ann['missing_annotations']}"
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Code quality analysis dashboard"
    )
    parser.add_argument(
        "--complexity", action="store_true", help="Show complexity only"
    )
    parser.add_argument(
        "--lint", action="store_true", help="Show lint summary only"
    )
    parser.add_argument(
        "--changed", action="store_true", help="Analyze changed files only"
    )
    args = parser.parse_args()

    if args.complexity:
        code = print_report(
            show_complexity=True, show_lint=False, changed_only=args.changed
        )
    elif args.lint:
        code = print_report(
            show_complexity=False, show_lint=True, changed_only=args.changed
        )
    else:
