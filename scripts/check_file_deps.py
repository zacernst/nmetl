...
Usage:
    uv run python scripts/check_file_deps.py src/nmetl/cli/pipeline.py
    uv run python scripts/check_file_deps.py --package nmetl src/nmetl/validation.py
...
def _module_names_from_path(file_path: Path, package_root: Path) -> list[str]:
    ...
    For ``src/nmetl/cli/pipeline.py`` returns:
      - "nmetl.cli.pipeline"
      - "help_system"
    """
    names: list[str] = []
...
def _find_consumers(
    ...
    for name in module_names:
        ...
        # string references like "from nmetl.X import Y" in comments, etc.)
