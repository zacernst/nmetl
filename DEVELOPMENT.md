...
nmetl/
├── packages/
│   ├── pycypher/       # Cypher parser, AST, query engine, ingestion layer
│   ├── nmetl/          # ETL layer: pipeline YAML config, validation, `nmetl` CLI
│   │   └── tests/      # CLI, config, sink, and REPL tests
│   └── shared/         # Common utilities, logging, telemetry
├── tests/              # Engine tests (flat structure, ~450 files)
...
Dependency order: `shared` → `pycypher` → `nmetl`.
`pycypher` never imports `nmetl`; the ETL layer sits strictly above the engine.
