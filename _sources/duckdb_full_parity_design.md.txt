...
Status: historical. Phases 0–3 landed as described; the relation engine's
query coverage is no longer a whitelist of shapes — see ADR-008 and
`docs/developer_guide/relation_plan.md` (2026-09-05).

Sibling: `duckdb_eager_path_design.md` covers the **eager path** — making
`ClauseExecutor`/`BindingFrame`'s scans, filters, renames, concats, index
construction, path expansion, and statistics run in DuckDB for queries that
are *not* relation-eligible. This document covers the **relation path**: the
whole-query compiler in `relation_engine.py` and the storage model beneath
it. The two efforts meet at the `_shadow` guards in `scan_operators.py`
(`:185`, `:360-363`), which keep a lazy scan from reading past a pending
pandas mutation overlay.
...
`packages/nmetl/tests/test_nmetl_run_streaming.py::TestMutationInterleavedWithRead`.
...
Note the tension with `duckdb_eager_path_design.md`, which plans to make the
eager path itself columnar rather than retire it. The two are alternative
routes to the same bounded-memory goal and are not mutually exclusive:
closing eligibility gaps here shrinks how often the eager path is reached,
while that plan lowers the cost of the times it still is. Whichever lands
first reduces — but does not eliminate — the value of the other, so pick by
which query shapes real pipelines actually hit, not by finishing this
phase's list for its own sake.
