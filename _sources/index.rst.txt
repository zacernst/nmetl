...
* **PyCypher** — openCypher query parser, AST models, and the BindingFrame
  / DuckDB execution engine, plus the ingestion layer that loads tabular
  data into a query context
* **nmetl** — the ETL layer on top of PyCypher: YAML pipeline configuration
  and validation, and the ``nmetl`` command-line tool that runs a pipeline
  of Cypher queries end to end
* **Shared** — Common utilities, logging, and telemetry

``nmetl`` depends on ``pycypher``; the engine never imports the ETL layer.
