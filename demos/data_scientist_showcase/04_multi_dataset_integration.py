...
def main() -> None:
    ...
    print(
        "With PyCypher, you just load each source as an entity or relationship.\n"
    )
    ...
    projects = pd.DataFrame(
        {
            ...
            "status": [
                rng2.choice(["Active", "Planning", "Completed", "On Hold"])
                for _ in range(10)
            ],
            "priority": [
                rng2.choice(["High", "Medium", "Low"]) for _ in range(10)
            ],
        }
    )
    ...
    for i, emp_id in enumerate(emp_ids):
        if i < len(emp_ids) // 2:
            ...
            mentees = rng4.sample(
                mentee_pool, min(n_mentees, len(mentee_pool))
            )
    ...
    print(
        "Five separate data sources → one graph. No schema alignment needed.\n"
    )
    ...
    with timed("Employee-project traversal"):
        ...
        """)
    show_result(
        result, label="Active project team composition (HR + PMO data fused)"
    )
    ...
    print(
        "This requires joining across HR mentorship + PMO assignment data.\n"
    )
    ...
    print(
        "Analyze departments — combining Finance budget data with HR records.\n"
    )
    ...
    with timed("Employee distribution"):
        ...
        """)
    show_result(
        result, label="Employee distribution by department and level (from HR)"
    )
    ...
    print(
        "Zoom into high-priority projects — who's on them and at what capacity.\n"
    )
    ...
    with timed("High-priority teams"):
        ...
        """)
    show_result(
        result, label="High-priority project teams (full cross-source detail)"
    )
    ...
    show_result(
        result,
        label="All employee-project assignments (spot multi-project people)",
    )
    ...
    print(
        "  5. DEPARTMENT ANALYSIS — Finance budgets + HR headcounts combined"
    )
