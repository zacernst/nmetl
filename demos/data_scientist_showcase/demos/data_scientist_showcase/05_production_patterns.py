...
def demo_query_validation() -> None:
    ...
    print("    if errors:")
    print(
        '        return {"status": "invalid", "errors": [str(e) for e in errors]}'
    )
...
def demo_timeout_protection(star: Star) -> None:
    ...
    print(
        f'        log.warning("Query timed out after %.1fs", e.elapsed_seconds)'
    )
...
def demo_result_caching(star: Star) -> None:
    ...
    query = (
        "MATCH (r:Record) WHERE r.category = 'alpha' RETURN r.name, r.value"
    )
    ...
    print(
        f"  First call (cache miss):  {len(result1)} rows in {first_ms:.1f}ms"
    )
    print(
        f"  Second call (cache hit):  {len(result2)} rows in {second_ms:.1f}ms"
    )
...
def demo_pipeline_api(star: Star) -> None:
    ...
    try:
        ...
        print(
            f"  Result rows: {len(result.result) if hasattr(result, 'result') else 'N/A'}"
        )
...
def demo_scaling() -> None:
    ...
    print()
    print(
        f"  {'Rows':>10s}  {'Build (ms)':>12s}  {'Query (ms)':>12s}  {'Results':>10s}"
    )
    print(f"  {'-' * 10}  {'-' * 12}  {'-' * 12}  {'-' * 10}")
    ...
    for n in sizes:
        ...
        print(
            f"  {n:>10,d}  {build_ms:>12.1f}  {query_ms:>12.1f}  {len(result):>10d}"
        )
