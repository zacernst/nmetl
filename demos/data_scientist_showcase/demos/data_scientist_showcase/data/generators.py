...
def scalable_relationships(
    entity_df: pd.DataFrame, density: float = 2.0
) -> pd.DataFrame:
...
def messy_awards(
    contractors_df: pd.DataFrame, agencies_df: pd.DataFrame
) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        {
            ...
            "priority": [
                rng.choice(["High", "Medium", "Low"]) for _ in range(n)
            ],
        }
    )
    ...
    nodes = pd.DataFrame(
        {
            ...
            "influence": [
                round(rng.uniform(0.1, 10.0), 2) for _ in range(n_nodes)
            ],
            "active": [
                rng.choice([True, True, True, False]) for _ in range(n_nodes)
            ],
        }
    )
