...
def main() -> None:
    ...
    contractors = pd.DataFrame(
        [
            ...
            {
                "__ID__": 9,
                "vendor_name": "Umbrella Corporation",
                "state": "ca",
                "amount": 1800000.00,
            ...
            {
                "__ID__": 13,
                "vendor_name": "Wayne Enterprises Inc.",
                "state": "NJ",
                "amount": 1450000.00,
    ...
    with timed("Case-normalized vendor query"):
        ...
        """)
    show_result(
        result, label="Vendors (normalized for display, raw data untouched)"
    )
    ...
    with timed("Variant name discovery"):
        ...
        """)
    show_result(
        result, label="All name variants for NAICS 541512 (IT Services)"
    )
    ...
    print(
        "Find all California contractors regardless of case ('CA', 'ca', 'Ca').\n"
    )
    ...
    show_result(
        result,
        label="California contractors (matched via toUpper normalization)",
    )
    ...
    print(
        'Award statuses are inconsistent: "Active", "ACTIVE", "active", etc.'
    )
    ...
    print(
        "  3. FOUND DUPLICATE VENDORS — NAICS grouping reveals name variants"
    )
    print(
        "  4. FILTERED ACROSS INCONSISTENCIES — case-insensitive state matching"
    )
    print(
        "  5. TRAVERSED RELATIONSHIPS — agency-contractor-state graph patterns"
    )
    print("  6. NORMALIZED STATUSES — Active/ACTIVE/active handled in queries")
    print(
        "  7. TEMPORAL ANALYSIS — fiscal year filtering without date parsing"
    )
    print()
