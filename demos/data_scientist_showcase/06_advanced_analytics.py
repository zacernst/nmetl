...
def demo_hub_detection(star: Star) -> None:
    ...
    show_result(
        result, label="  Most sought-after nodes (by in-degree)", max_rows=10
    )
...
def demo_cross_cluster(star: Star) -> None:
    ...
    show_result(
        result, label="  Bridge nodes (most cross-cluster links)", max_rows=10
    )
...
def demo_connection_patterns(star: Star) -> None:
    ...
    show_result(
        result,
        label="  Strong cross-cluster connections (strength > 0.7)",
        max_rows=10,
    )
...
def demo_network_summary(star: Star) -> None:
    ...
    edge_count = star.execute_query(
        "MATCH ()-[c:CONNECTS]->() RETURN count(c) AS cnt"
    )
...
def main() -> None:
    ...
    print(
        f"Graph loaded: {node_count.iloc[0]['cnt']} nodes, "
    ...
    capabilities = [
        (
            ...
            "Paths of arbitrary depth, naturally expressed",
        ),
        ...
        (
            ...
            "Multi-stage WITH chains for complex analytics",
        ),
