...
def _large_config(
    n_entities=50, n_rels=30, n_queries=0, n_outputs=0
) -> PipelineConfig:
    return _config(
        entities=[
            EntitySourceConfig(
        ...
        queries=[
            QueryConfig(id=f"q{i}", inline=f"MATCH (n) RETURN n LIMIT {i}")
...
class TestViewModelContracts:
    ...
    def test_relationship_view_model_column_mappings_are_tuple(self):
        vm = RelationshipViewModel(
            ...
            source_count=1,
            column_mappings=(
                ColumnMappingViewModel(source_col="pid", target_col="fid"),
            ),
...
class TestValidationContracts:
    ...
    def test_entity_duplicate_source_id_rejected_at_config_level(self):
        ...
        with pytest.raises(ValidationError, match="Duplicate source id"):
            _config(
                entities=[
                    EntitySourceConfig(
                        id="same_id", uri="a.csv", entity_type="Person"
                    ),
                    EntitySourceConfig(
                        id="same_id", uri="b.csv", entity_type="Person"
                    ),
                ]
            )
    ...
    def test_entity_with_valid_uri_has_no_uri_errors(self):
        config = _config(
            entities=[
                EntitySourceConfig(
                    id="e1", uri="data/people.csv", entity_type="Person"
                ),
            ]
        )
        adapter = _adapter(config)
    ...
    def test_relationship_unresolved_source_col_produces_warning(self):
        ...
        warnings = [
            i for i in detail.validation_issues if i.level == "warning"
        ]
    ...
    def test_validation_issue_supports_fix_hint_field(self):
        ...
        issue = ValidationIssue(
            ...
            message="URI missing",
            field="uri",
            fix_hint="Provide a valid file path",
        )
    ...
    def test_fully_valid_config_has_no_issues(self):
        config = _config(
            entities=[
                EntitySourceConfig(
                    id="e1", uri="a.csv", entity_type="Person", id_col="pid"
                ),
            ],
            relationships=[
                RelationshipSourceConfig(
...
class TestStatisticsContracts:
    ...
    def test_stats_count_unique_types_not_sources(self):
        ...
        config = _config(
            entities=[
                ...
                EntitySourceConfig(
                    id="e3", uri="c.csv", entity_type="Company"
                ),
            ]
        )
        adapter = _adapter(config)
    ...
    def test_stats_with_mock_index_manager(self):
        ...
        config = _config(
            entities=[
                EntitySourceConfig(
                    id="e1", uri="p.csv", entity_type="Person", id_col="pid"
                ),
            ]
        )
...
class _ConcreteEditableScreen(VimEditableScreen[_TestItem]):
    ...
    def get_fields(self, item):
        ...
        return [
            EditableField(
                name="name", label="Name", value=item.name, readonly=True
            ),
...
class TestGracefulDegradation:
    ...
    def test_broken_index_manager_does_not_crash(self):
        ...
        broken_idx = Mock()
        type(broken_idx)._label = property(
