...
class TestKeyResult:
    ...
    def test_transition_result(self):
        r = KeyResult(handled=True, transition_to=ModeType.INSERT)
...
class TestModeManager:
    ...
    def test_transition_listener_called(self):
        ...
        mgr.add_listener(lambda old, new: transitions.append((old, new)))
        mgr.transition_to(ModeType.INSERT)
        assert transitions == [(ModeType.NORMAL, ModeType.INSERT)]
...
class TestVisualMode:
    ...
    def test_hjkl_extends_selection(self, visual):
        for key, direction in [
            ...
            assert result.handled is True
            assert result.command == f"selection:extend_{direction}"
