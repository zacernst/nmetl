    ...
    async def test_handle_extra_key_returns_true_for_shift_tab(self):
        ...
        async with app.run_test() as pilot:
            ...
            assert result is True, (
                "handle_extra_key did not consume 'shift+tab'"
            )
...
class TestTabNavigationRegressionGuards:
    ...
    async def test_regression_tabs_are_navigable_not_just_visible(self):
        ...
        async with app.run_test() as pilot:
            ...
            await pilot.pause()
            assert tabs.active == "tab-attributes", (
                "Tab switch to Attributes failed"
            )

            tabs.active = "tab-validation"
            await pilot.pause()
            assert tabs.active == "tab-validation", (
                "Tab switch to Validation failed"
            )

            tabs.active = "tab-statistics"
            await pilot.pause()
            assert tabs.active == "tab-statistics", (
                "Tab switch to Statistics failed"
            )
            ...
            await pilot.pause()
            assert tabs.active == "tab-overview", (
                "Tab switch back to Overview failed"
            )
...
class TestTabContentCorrectness:
    ...
    @pytest.mark.asyncio
    async def test_switching_between_entity_and_relationship_updates_overview(
        self,
    ):
