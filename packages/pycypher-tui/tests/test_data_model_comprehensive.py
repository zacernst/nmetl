    ...
    def test_entities_only(self):
        config = PipelineConfig(
            sources=SourcesConfig(
                entities=[
                    EntitySourceConfig(
                        id="e1", uri="f:///a.csv", entity_type="A", id_col="id"
                    ),
                    EntitySourceConfig(
                        id="e2", uri="f:///b.csv", entity_type="B", id_col="id"
                    ),
                ],
                relationships=[],
            ),
...
class TestSourceMappingValidation:
    ...
    def test_entity_without_id_column_warns(self):
        ...
        assert any(
            "ID column" in issue["message"] for issue in result["issues"]
        )
    ...
    def test_entity_single_column_warns(self):
        ...
        assert result["status"] == "warning"
        assert any(
            "one column" in issue["message"].lower()
    ...
    def test_relationship_missing_source_col_errors(self):
        ...
        assert any(
            "person_id" in issue["message"] for issue in result["issues"]
        )
    ...
    def test_relationship_missing_target_col_errors(self):
        ...
        assert result["status"] == "error"
        assert any(
            "friend_id" in issue["message"] for issue in result["issues"]
        )
    ...
    def test_validation_exception_returns_error(self):
        ...
        schema_bad = Mock()
        type(schema_bad).columns = property(
            lambda self: (_ for _ in ()).throw(RuntimeError("broken"))
        )
        node = _entity_node()
...
class TestDataModelScreenDisplay:
    ...
    async def test_ecommerce_shows_all_entity_types(self):
        ...
        async with app.run_test() as pilot:
            ...
            entity_labels = {
                n.node.label for n in nodes if n.node.node_type == "entity"
            }
    ...
    async def test_ecommerce_shows_relationships(self):
        ...
        async with app.run_test() as pilot:
            ...
            rel_labels = {
                n.node.label
                for n in nodes
                if n.node.node_type == "relationship"
            }
