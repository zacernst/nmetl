...
class TestAttributeLoadingAsync:
    ...
    async def test_loads_attribute_data_for_entity(self):
        ...
        mock_stats = {
            "id": Mock(dtype="int64", null_count=0, unique_count=100)
        }
        mock_sample_df = Mock()
        mock_sample_df.to_dict.return_value = [{"id": 1}, {"id": 2}]

        with patch(
            "pycypher_tui.screens.data_model.DataSourceIntrospector"
    ...
    async def test_handles_introspection_error_gracefully(self):
        ...
        with patch(
            ...
            mock_introspector_class.side_effect = Exception(
                "Connection failed"
            )
...
class TestAttributeValidation:
    ...
    def test_validates_entity_id_column(self):
        ...
        if result.get("issues"):
            id_warning = any(
                "id" in issue["message"].lower() for issue in result["issues"]
            )
    ...
    def test_validates_relationship_columns(self):
        ...
        target_error = any(
            "product_id" in issue["message"] for issue in result["issues"]
        )
        assert target_error
    ...
    def test_validation_handles_exception(self):
        ...
        mock_source.source_col.__contains__ = Mock(
...
class TestAttributeInspectorWorkflow:
    ...
    async def test_navigation_updates_attribute_inspector(self):
        ...
        async with app.run_test() as pilot:
            ...
            initial_labels = [
                str(l.render()) for l in overview_tab.query(Label)
            ]
