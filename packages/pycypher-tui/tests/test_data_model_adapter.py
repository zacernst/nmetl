...
class TestEntityAccess:
    ...
    def test_multiple_entity_types_sorted(self):
        config = _make_config(
            entities=[
                EntitySourceConfig(
                    id="c1", uri="c.csv", entity_type="Company"
                ),
                EntitySourceConfig(id="p1", uri="p.csv", entity_type="Person"),
                EntitySourceConfig(
                    id="a1", uri="a.csv", entity_type="Address"
                ),
            ]
        )
        adapter = _make_adapter(config)
        ...
        assert len(result) == 3
        assert [r.entity_type for r in result] == [
            "Address",
            "Company",
            "Person",
        ]
...
class TestRelationshipAccess:
    ...
    def test_relationship_detail(self):
        config = _make_config(
            entities=[
                EntitySourceConfig(
                    id="p1", uri="p.csv", entity_type="Person", id_col="pid"
                ),
            ],
            relationships=[
                RelationshipSourceConfig(
...
class TestSourceMappings:
    def test_entity_and_relationship_sources(self):
        ...
        rel_mapping = [m for m in result if m.mapping_type == "relationship"][
            0
        ]
