...
class TestColumnMappingWidget:
    ...
    @patch(
        "pycypher_tui.widgets.column_mapping.DataSourceIntrospector",
        MockIntrospector,
    )
    async def test_update_relationship_sources_valid(
        self, column_mapping_widget, sample_relationship_sources
    ):
        ...
        )  # Columns don't exist in mock schema

    @patch(
        "pycypher_tui.widgets.column_mapping.DataSourceIntrospector",
        MockIntrospector,
    )
    async def test_update_relationship_sources_invalid(
        self, column_mapping_widget, sample_relationship_sources
    ):
    ...
    def test_types_compatible(self, column_mapping_widget):
        ...
        # Incompatible types
        assert not column_mapping_widget._types_compatible(
            "integer", "varchar"
        )
    ...
    def test_validate_mapping_missing_columns(self, column_mapping_widget):
        ...
        status, message = column_mapping_widget._validate_mapping(
    ...
    def test_navigation_key_handling(self, column_mapping_widget):
        ...
        # Test moving up
        column_mapping_widget.selected_index = max(
            column_mapping_widget.selected_index - 1, 0
        )
