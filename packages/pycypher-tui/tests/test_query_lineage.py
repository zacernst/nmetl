...
def test_analyze_pipeline_lineage(sample_config):
    ...
    assert (
        len(analysis.components) == 5
    )  # 2 entities + 1 relationship + 1 query + 1 output
    ...
    output_comp = next(
        c for c in analysis.components if c.component_type == "output"
    )
...
def test_generate_flow_diagram(sample_config):
    ...
    analysis = _analyze_pipeline_lineage(sample_config)
    diagram = _generate_flow_diagram(
        analysis.components, analysis.dependency_graph
    )
...
def test_pipeline_component_search_text(config_manager):
    ...
    entity_comp = next(
        c
        for c in items
        if c.component_type == "source" and "Entity:" in c.display_name
    )
...
def test_component_metadata_extraction(sample_config):
    ...
    entity_comp = next(
        c for c in analysis.components if c.display_name == "Entity: Person"
    )
    ...
    rel_comp = next(
        ...
        if c.display_name == "Relationship: PURCHASED"
    )
    ...
    query_comp = next(
        c for c in analysis.components if c.display_name == "Query: user_stats"
    )
    ...
    output_comp = next(
        c for c in analysis.components if c.display_name.startswith("Output:")
    )
...
def test_dependency_relationships(sample_config):
    ...
    query_comp = next(
        c for c in analysis.components if c.component_type == "query"
    )
    ...
    query_comp = next(
        c for c in analysis.components if c.component_id == "query:user_stats"
    )
