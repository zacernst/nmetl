...
class NormalMode(BaseMode):
    ...
    def handle_key(self, key: str) -> KeyResult:
        ...
        match key:
            # Navigation
            case "h" | "left":
                return KeyResult(handled=True, command="navigate:left")
            case "j" | "down":
                return KeyResult(handled=True, command="navigate:down")
            case "k" | "up":
                return KeyResult(handled=True, command="navigate:up")
            case "l" | "right":
                return KeyResult(handled=True, command="navigate:right")

            # Word motions
            case "w":
                return KeyResult(handled=True, command="motion:word_forward")
            case "b":
                return KeyResult(handled=True, command="motion:word_backward")
            case "e":
                return KeyResult(handled=True, command="motion:word_end")
            ...
            # Jump navigation
            case "G":
                return KeyResult(handled=True, command="navigate:last")
            case "g":
            ...
            # Operations
            case "y":
                return KeyResult(handled=True, command="clipboard:yank")
            case "p":
                return KeyResult(handled=True, command="clipboard:paste")
            case "d":
            ...
            case "c":
                ...
                return KeyResult(handled=True, pending=True)
            case "u":
                return KeyResult(handled=True, command="edit:undo")
            case "ctrl+r":
                return KeyResult(handled=True, command="edit:redo")
            case "enter":
                return KeyResult(handled=True, command="action:confirm")
            case "escape":
                self._pending_keys.clear()
    ...
    def _handle_pending(self, key: str) -> KeyResult:
        ...
        match sequence:
            case "gg":
                return KeyResult(handled=True, command="navigate:first")
            case "dd":
                return KeyResult(handled=True, command="edit:delete_line")
            case _:
                return KeyResult(handled=False)
