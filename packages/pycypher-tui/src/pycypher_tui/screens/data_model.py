...
class ModelNodeWidget(BaseListItem[ModelNode]):
    ...
    def compose(self) -> ComposeResult:
        if self.node.node_type == "entity":
            ...
            sources_text = f"{self.node.source_count} source{'s' if self.node.source_count != 1 else ''}"
            yield Label(
...
class ModelDetailPanel(BaseDetailPanel):
    ...
    def _update_overview_tab(self, node: ModelNode | None) -> None:
        ...
        try:
            ...
            if node.node_type == "entity":
                container.mount(
                    Label(f"Entity: {node.label}", classes="section-header")
                )
            else:
                container.mount(
                    Label(
                        f"Relationship: :{node.label}",
                        classes="section-header",
                    )
                )

            container.mount(
                Label(f"Type: {node.node_type}", classes="info-row")
            )

            sources_text = f"{node.source_count} data source{'s' if node.source_count != 1 else ''}"
            container.mount(
                Label(f"Sources: {sources_text}", classes="info-row")
            )
        ...
        try:
            ...
            if node.node_type == "entity":
                sources = [
                    e
                    for e in config.sources.entities
                    if e.entity_type == node.label
                ]
            else:
                sources = [
                    r
                    for r in config.sources.relationships
                    if r.relationship_type == node.label
                ]
            ...
            for source in sources:
                try:
                    ...
                    validation_results[source.id] = (
                        self._validate_source_mapping(source, schema, node)
                    )

                except Exception as exc:
                    logger.warning(
                        "Failed to introspect source %s: %s", source.id, exc
                    )
                    validation_results[source.id] = {
                        "status": "error",
            ...
            logger.exception(
                "Failed to load attribute data for %s", node.label
            )
            self._attribute_data = AttributeData(error=str(exc))
            self.call_after_refresh(self._update_attribute_display)

    def _validate_source_mapping(
        self, source, schema, node: ModelNode
    ) -> dict:
        ...
        try:
            ...
            if node.node_type == "entity":
                ...
                id_candidates = [
                    col for col in column_names if "id" in col.lower()
                ]
                ...
                if (
                    hasattr(source, "source_col")
                    and source.source_col not in column_names
                ):
                    validation["issues"].append(
                        {
                            "type": "error",
                ...
                if (
                    hasattr(source, "target_col")
                    and source.target_col not in column_names
                ):
                    validation["issues"].append(
                        {
                            "type": "error",
            ...
            if any(issue["type"] == "error" for issue in validation["issues"]):
                ...
                issue["type"] == "warning" for issue in validation["issues"]
            ):
            ...
            validation = {
                "status": "error",
                "issues": [
                    {"type": "error", "message": f"Validation failed: {exc}"}
                ],
            }
    ...
    def _update_attributes_tab(self) -> None:
        ...
        try:
            ...
            for source_id, schema in self._attribute_data.schema_info.items():
                container.mount(
                    Label(f"Source: {source_id}", classes="section-header")
                )
                container.mount(
                    Label(f"Rows: {schema['row_count']:,}", classes="info-row")
                )
                container.mount(
                    Label(
    ...
    def _update_validation_tab(self) -> None:
        ...
        if (
            not self._attribute_data
            or not self._attribute_data.validation_results
        ):
            return
        ...
        try:
            ...
            if (
                self._current_node
                and self._current_node.node_type == "relationship"
            ):
    ...
    def _update_entity_validation_tab(self, tab) -> None:
        ...
        for (
            source_id,
            validation,
        ) in self._attribute_data.validation_results.items():
            container.mount(
                Label(f"Source: {source_id}", classes="section-header")
            )
            ...
            if "issues" in validation and validation["issues"]:
                ...
                container.mount(
                    Label("No issues found", classes="validation-pass")
                )
        ...
        try:
            ...
            if self._current_relationship_sources:
                ...
                self.config_manager.update_relationship_source(
                    source.id,
                    source_col=new_mapping.get(
                        "source_col", source.source_col
                    ),
                    target_col=new_mapping.get(
        ...
        try:
            ...
            for source_id, stats in self._attribute_data.column_stats.items():
                ...
                for col_name, col_stats in stats.items():
                    container.mount(
                        Label(f"{col_name}:", classes="info-label")
                    )
                    container.mount(
                        Label(f"  Type: {col_stats.dtype}", classes="info-row")
                    )
                    container.mount(
                        Label(
                    ...
                    container.mount(
                        Label(
                            f"  Unique Count: {col_stats.unique_count:,}",
                            classes="info-row",
                        )
                    )
    ...
    def _update_lineage_tab(self) -> None:
        ...
        try:
            ...
            if self._current_node:
                ...
                container.mount(
                    Label(
                        f"Node Type: {self._current_node.node_type}",
                        classes="info-row",
                    )
                )
                container.mount(
                    Label(
                        f"Source Count: {self._current_node.source_count}",
                        classes="info-row",
                    )
                )
                ...
                if self._current_node.source_ids:
                    container.mount(
                        Label("Data Sources:", classes="info-label")
                    )
                    for source_id in self._current_node.source_ids:
                        container.mount(
                            Label(f"  → {source_id}", classes="info-row")
                        )

                if self._current_node.connections:
                    container.mount(
                        Label("Graph Connections:", classes="info-label")
                    )
                ...
                container.mount(
                    Label(
                        "Advanced lineage tracking coming soon...",
                        classes="info-row",
                    )
                )
...
class DataModelScreen(VimNavigableScreen[ModelNode]):
    ...
    def empty_list_message(self) -> str:
        return (
            "No entity or relationship types defined.\nAdd data sources first."
        )
    ...
    def update_detail_panel(self, item: ModelNode | None) -> None:
        try:
            detail = self.query_one(
                f"#{self.detail_panel_id}", ModelDetailPanel
            )
