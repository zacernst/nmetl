...
class RelationshipBrowserDetailPanel(BaseDetailPanel):
    ...
    def __init__(self, adapter: DataModelAdapter, **kwargs) -> None:
        super().__init__(
            empty_message="(no relationship type selected)", **kwargs
        )
    ...
    def update_relationship(self, rel: RelationshipViewModel | None) -> None:
        ...
        if rel is None:
            self.mount(
                Label(
                    "(no relationship type selected)", classes="detail-title"
                )
            )
            return
        ...
        container.mount(
            Label(
                f"Relationship: {detail.relationship_type}",
        ...
        if detail.column_mappings:
            container.mount(
                Label("Column Mappings:", classes="detail-section")
            )
        ...
        if detail.validation_issues:
            ...
            container.mount(Label("Validation:", classes="detail-section"))
            container.mount(
                Label("  [ok] No issues found", classes="validation-pass")
            )
...
class RelationshipBrowserScreen(VimNavigableScreen[RelationshipViewModel]):
    ...
    def _update_summary(self) -> None:
        ...
        warnings = sum(
            1 for r in self.items if r.validation_status == "warning"
        )
        ...
        if total == 0:
            ...
            summary_widget.update(
                f" {total} relationship types, {errors} with errors"
            )
