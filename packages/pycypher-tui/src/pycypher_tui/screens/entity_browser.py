...
class EntityListItem(BaseListItem[EntityViewModel]):
    ...
    def compose(self) -> ComposeResult:
        ...
        id_text = (
            f"  ID: {self.entity.id_column}" if self.entity.id_column else ""
        )
        yield Label(
...
class EntityBrowserDetailPanel(BaseDetailPanel):
    ...
    def update_entity(self, entity: EntityViewModel | None) -> None:
        ...
        if entity is None:
            self.mount(
                Label("(no entity type selected)", classes="detail-title")
            )
            return
        ...
        # Header
        container.mount(
            Label(f"Entity: {detail.entity_type}", classes="detail-title")
        )
        ...
        if detail.properties:
            ...
            for prop in detail.properties:
                source_note = (
                    f" (from {prop.source_id})" if prop.source_id else ""
                )
                container.mount(
                    Label(
...
class EntityBrowserScreen(VimNavigableScreen[EntityViewModel]):
    ...
    def create_list_item(
        self, item: EntityViewModel, item_id: str
    ) -> BaseListItem:
