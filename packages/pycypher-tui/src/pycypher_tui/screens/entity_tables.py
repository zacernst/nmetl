...
class EntityDetailPanel(BaseDetailPanel):
    ...
    def update_entity(self, entity: EntityTableInfo | None) -> None:
        ...
        self.mount(
            Label(f"  Source: {entity.source_id}", classes="detail-row")
        )
        ...
        if entity.id_col:
            self.mount(
                Label(
                    f"  ID Col: {entity.id_col} -> __ID__", classes="column-id"
                )
            )
        else:
            self.mount(
                Label("  ID Col: (auto-generated)", classes="detail-row")
            )

        if entity.has_query:
            self.mount(
                Label("  Query:  (custom SQL filter)", classes="detail-row")
            )
        ...
        if entity.schema_hints:
            ...
            for col_name, col_type in entity.schema_hints.items():
                self.mount(
                    Label(
                        f"  {col_name}: {col_type}", classes="column-property"
                    )
                )
...
class EntityListItem(BaseListItem[EntityTableInfo]):
    ...
    def compose(self) -> ComposeResult:
        id_marker = (
            f" [ID: {self.entity.id_col}]" if self.entity.id_col else ""
        )
        yield Label(
...
class EntityTablesScreen(VimNavigableScreen[EntityTableInfo]):
    ...
    def create_list_item(
        self, item: EntityTableInfo, item_id: str
    ) -> BaseListItem:
    ...
    def update_detail_panel(self, item: EntityTableInfo | None) -> None:
        try:
            detail = self.query_one(
                f"#{self.detail_panel_id}", EntityDetailPanel
            )
