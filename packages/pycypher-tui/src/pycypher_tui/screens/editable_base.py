...
class ValidationPanel(Static):
    ...
    def update_issues(self, issues: list[tuple[str, str, str]]) -> None:
        ...
        if not issues:
            self.mount(
                Label("[ok] All fields valid", classes="validation-pass")
            )
            return
...
class VimEditableScreen(VimNavigableScreen[T], Generic[T]):
    ...
    @abstractmethod
    def apply_changes(
        self, item: T | None, field_values: dict[str, str]
    ) -> None:
    ...
    def validate_all_fields(self) -> bool:
        ...
        for widget in self._field_widgets:
            ...
            if result.valid:
                issues.append(
                    ("pass", "[ok]", f"{widget.field_def.label} valid")
                )
            else:
                all_valid = False
                issues.append(
                    (
                        "error",
                        "[XX]",
                        f"{widget.field_def.label}: {result.error}",
                    )
                )
