...
class SourceDetailPanel(BaseDetailPanel):
    ...
    def update_source(self, source: SourceItem | None) -> None:
        ...
        self.mount(
            Label(f"  Type: {source.source_type}", classes="detail-row")
        )
        ...
        if source.id_col:
            self.mount(
                Label(f"  ID Column: {source.id_col}", classes="detail-row")
            )
...
class SourceListItem(BaseListItem[SourceItem]):
    ...
    def compose(self) -> ComposeResult:
        type_indicator = (
            "[E]" if self.source.source_type == "entity" else "[R]"
        )
        yield Label(
...
class DataSourcesScreen(VimNavigableScreen[SourceItem]):
    ...
    def load_items(self) -> list[SourceItem]:
        ...
        if self._filter_mode == "entity":
            ...
            all_sources = [
                s for s in all_sources if s.source_type == "relationship"
            ]
    ...
    def update_detail_panel(self, item: SourceItem | None) -> None:
        try:
            detail = self.query_one(
                f"#{self.detail_panel_id}", SourceDetailPanel
            )
    ...
    def _edit_source_uri(self, item: SourceItem) -> None:
        ...
        def _got_uri(response):
            ...
            if item.source_type == "entity":
                self.config_manager.update_entity_source(
                    item.source_id, uri=new_uri
                )
            else:
            ...
            self.post_message(
                self.EditSource(item.source_id, item.source_type)
            )
    ...
    def on_add(self) -> None:
        source_type = (
            "entity" if self._filter_mode != "relationship" else "relationship"
        )
    ...
    def _add_source_step2(self, source_type: str, source_id: str) -> None:
        ...
        def _got_uri(response):
            if response.result != DialogResult.CONFIRMED or not response.value:
                return
            self._add_source_step3(
                source_type, source_id, response.value.strip()
            )
    ...
    def _add_source_step3(
        self, source_type: str, source_id: str, uri: str
    ) -> None:
        ...
        label = (
            "entity type" if source_type == "entity" else "relationship type"
        )
        self.app.push_screen(
            InputDialog(
                title=f"Add {source_type.title()} Source",
    ...
    def _mount_filter_indicator(self) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    "_mount_filter_indicator: #screen-header not found"
                )
    ...
    async def _cycle_filter(self) -> None:
        ...
        self._filter_mode = self.FILTER_MODES[
            (idx + 1) % len(self.FILTER_MODES)
        ]
        await self.refresh_from_config()
    ...
    def _update_filter_indicator(self) -> None:
        ...
        try:
            ...
            logger.debug(
                "_update_filter_indicator: #filter-indicator not found"
            )
