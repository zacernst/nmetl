...
class TemplateDetailPanel(BaseDetailPanel):
    ...
    def update_template(self, summary: TemplateSummary | None) -> None:
        ...
        self.mount(
            Label(f"  Category: {summary.category}", classes="detail-category")
        )
        self.mount(Label("", classes="detail-row"))
        self.mount(Label("  Includes:", classes="detail-row"))
        self.mount(
            Label(
                f"    {summary.entity_count} entity sources",
                classes="detail-stat",
            )
        )
        self.mount(
            Label(
                f"    {summary.relationship_count} relationship sources",
                classes="detail-stat",
            )
        )
        self.mount(
            Label(f"    {summary.query_count} queries", classes="detail-stat")
        )
        self.mount(
            Label(f"    {summary.output_count} outputs", classes="detail-stat")
        )
...
class TemplateBrowserScreen(VimNavigableScreen[TemplateSummary]):
    ...
    def __init__(
        self, config_manager: ConfigManager | None = None, **kwargs
    ) -> None:
        ...
        super().__init__(
            config_manager=config_manager or ConfigManager(), **kwargs
        )
    ...
    def load_items(self) -> list[TemplateSummary]:
        ...
        if self._category_filter:
            return [
                s for s in all_summaries if s.category == self._category_filter
            ]
        return all_summaries

    def create_list_item(
        self, item: TemplateSummary, item_id: str
    ) -> BaseListItem:
    ...
    def update_detail_panel(self, item: TemplateSummary | None) -> None:
        try:
            detail = self.query_one(
                f"#{self.detail_panel_id}", TemplateDetailPanel
            )
