...
class BaseDetailPanel(Static):
    ...
    def __init__(
        self, empty_message: str = "(no item selected)", **kwargs
    ) -> None:
...
class VimNavigableScreen(Widget, Generic[T]):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(id="screen-main"):
            with VerticalScroll(id=self.list_panel_id):
                if not self._items:
                    yield Label(
                        self.empty_list_message, classes="empty-list-message"
                    )
                else:
                    for i, item in enumerate(self._items):
                        item_id = self.get_item_id(item)
    ...
    def on_mount(self) -> None:
        ...
        try:
            ...
            logger.debug(
                "on_mount: list panel #%s not found for focus",
                self.list_panel_id,
            )
    ...
    async def _render_list(self) -> None:
        ...
        try:
            ...
            logger.debug(
                "_render_list: list panel #%s not found", self.list_panel_id
            )
            return
        ...
        if not self._items:
            await panel.mount(
                Label(self.empty_list_message, classes="empty-list-message")
            )
            return
                ...
                if 0 <= idx < self.item_count:
                    ...
                    try:
                        widget = self.query_one(
                            f"#item-{item_id}", BaseListItem
                        )
                        widget.focused = focused
                    except NoMatches, AttributeError:
                        logger.debug(
                            "_update_focus: widget #item-%s not found", item_id
                        )
        else:
            for i, item in enumerate(self._items):
                item_id = self.get_item_id(item)
    ...
    def _all_override_keys(self) -> frozenset[str]:
        ...
        try:
            ...
            self.__cached_override_keys = (
                self._SCREEN_OVERRIDE_KEYS | self._screen_override_keys
            )
            return self.__cached_override_keys
