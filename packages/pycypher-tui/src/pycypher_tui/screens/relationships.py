...
class RelationshipDetailPanel(BaseDetailPanel):
    ...
    def update_relationship(self, rel: RelationshipItem | None) -> None:
        ...
        if rel is None:
            self.mount(
                Label("(no relationship selected)", classes="detail-title")
            )
            return
        ...
        self.mount(
            Label(f"  Source column: {rel.source_col}", classes="detail-row")
        )
        self.mount(
            Label(f"  Target column: {rel.target_col}", classes="detail-row")
        )

        if rel.id_col:
            self.mount(
                Label(f"  ID column:     {rel.id_col}", classes="detail-row")
            )
...
class RelationshipScreen(VimNavigableScreen[RelationshipItem]):
    ...
    def create_list_item(
        self, item: RelationshipItem, item_id: str
    ) -> BaseListItem:
    ...
    def update_detail_panel(self, item: RelationshipItem | None) -> None:
        try:
            detail = self.query_one(
                f"#{self.detail_panel_id}", RelationshipDetailPanel
            )
    ...
    def _add_rel_step4(self, source_id: str, uri: str, rel_type: str) -> None:
        def _got_source_col(response):
            if response.result != DialogResult.CONFIRMED or not response.value:
                return
            self._add_rel_step5(
                source_id, uri, rel_type, response.value.strip()
            )
