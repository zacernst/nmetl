...
class SourceMappingDetailPanel(BaseDetailPanel):
    ...
    def update_mapping(self, mapping: SourceMappingViewModel | None) -> None:
        ...
        container.mount(
            Label(
                f"  Entity types: {stats.entity_type_count}",
                classes="detail-row",
            )
        )
        container.mount(
        ...
        container.mount(
            Label(
                f"  Total sources: {stats.total_source_count}",
                classes="detail-row",
            )
        )
        ...
        container.mount(
            Label("Press 'p' to preview data", classes="detail-row")
        )
...
class DataSourceMapperScreen(VimNavigableScreen[SourceMappingViewModel]):
    ...
    def get_item_search_text(self, item: SourceMappingViewModel) -> str:
        return (
            f"{item.source_id} {item.uri} {item.maps_to} {item.mapping_type}"
        )

    def on_edit(self, item: SourceMappingViewModel) -> None:
        self.post_message(self.DrillDown(item.source_id, item.mapping_type))
