...
def _analyze_pipeline_lineage(config) -> LineageAnalysis:
    ...
    try:
        ...
        for comp in components:
            ...
            components_with_dependents.append(
                PipelineComponent(
        ...
        flow_diagram = _generate_flow_diagram(
            components_with_dependents, dependency_graph
        )
...
def _generate_flow_diagram(
    components: list[PipelineComponent], dependency_graph: dict[str, list[str]]
) -> str:
    ...
    if sources:
        ...
        for source in sources:
            name = source.display_name.replace("Entity: ", "").replace(
                "Relationship: ", ""
            )
            ...
            if source.dependents:
                ...
                for i, dep in enumerate(source.dependents):
                    dep_comp = next(
                        (c for c in components if c.component_id == dep), None
                    )
                    if dep_comp:
                        dep_name = dep_comp.display_name.replace("Query: ", "")
                        connector = (
                            "└──>"
                            if i == len(source.dependents) - 1
                            else "├──>"
                        )
        ...
        for query in queries:
            ...
            if query.dependents:
                ...
                for dep in query.dependents:
                    ...
                    if dep_comp:
                        dep_name = dep_comp.display_name.replace(
                            "Output: ", ""
                        )
                        diagram_lines.append(f"  │    → {dep_name}")
            ...
            if output.dependencies:
                dep_comp = next(
                    (
                        c
                        for c in components
                        if c.component_id == output.dependencies[0]
                    ),
                    None,
                )
                if dep_comp:
...
def _find_critical_path(dependency_graph: dict[str, list[str]]) -> list[str]:
    ...
    sources = [
        comp_id for comp_id, deps in dependency_graph.items() if not deps
    ]
    longest_path = []
...
class LineageDetailPanel(BaseDetailPanel):
    ...
    def update_component(
        ...
        lineage_analysis: LineageAnalysis | None = None,
    ) -> None:
        ...
        self._worker = self.run_worker(
            self._load_component_details(), exclusive=True
        )

    def _update_overview_tab(
        self, component: PipelineComponent | None
    ) -> None:
        ...
        try:
            ...
            # Component header
            container.mount(
                Label(f"{component.display_name}", classes="section-header")
            )

            # Basic info
            container.mount(
                Label(f"Type: {component.component_type}", classes="info-row")
            )
            container.mount(
                Label(f"ID: {component.component_id}", classes="info-row")
            )
            ...
            container.mount(
                Label(
                    f"  Inputs: {len(component.dependencies)}",
                    classes="info-row",
                )
            )
            container.mount(
                Label(
                    f"  Outputs: {len(component.dependents)}",
                    classes="info-row",
                )
            )
            ...
            if component.metadata:
                ...
                for key, value in component.metadata.items():
                    if value is not None:
                        container.mount(
                            Label(f"  {key}: {value}", classes="info-row")
                        )
    ...
    def _show_loading_indicators(self) -> None:
        ...
        for tab_id in ["tab-dependencies", "tab-flow", "tab-analysis"]:
            try:
                ...
                logger.debug(
                    "Failed to show loading indicator in tab %s", tab_id
                )
    ...
    def _update_dependencies_tab(self) -> None:
        ...
        try:
            ...
            # Input dependencies
            container.mount(
                Label("Input Dependencies:", classes="section-header")
            )
            if component.dependencies:
                ...
                for dep_id in component.dependencies:
                    dep_comp = next(
                        (
                            c
                            for c in self._lineage_analysis.components
                            if c.component_id == dep_id
                        ),
                        None,
                    )
                    if dep_comp:
                ...
                container.mount(
                    Label("  No input dependencies", classes="info-row")
                )

            # Output dependents
            container.mount(Label("", classes="info-row"))  # spacer
            container.mount(
                Label("Output Dependents:", classes="section-header")
            )
            if component.dependents:
                ...
                container.mount(
                    Label("  No output dependents", classes="info-row")
                )
    ...
    def _update_flow_tab(self) -> None:
        ...
        try:
            ...
            container.mount(
                Label("Pipeline Flow Diagram:", classes="section-header")
            )
            container.mount(
                Static(
                    self._lineage_analysis.flow_diagram, classes="flow-diagram"
                )
            )
    ...
    def _update_analysis_tab(self) -> None:
        ...
        try:
            ...
            # Critical path
            container.mount(
                Label("Critical Path Analysis:", classes="section-header")
            )
            if self._lineage_analysis.critical_path:
                container.mount(
                    Label("Longest dependency chain:", classes="info-label")
                )
                for i, comp_id in enumerate(
                    self._lineage_analysis.critical_path
                ):
                    comp = next(
                        (
                            c
                    ...
                    if comp:
                        arrow = (
                            " → "
                            if i
                            < len(self._lineage_analysis.critical_path) - 1
                            else ""
                        )
                        container.mount(
                            Label(
                                f"  {comp.display_name}{arrow}",
                                classes="critical-path",
                            )
                        )
            else:
                container.mount(
                    Label("  No critical path found", classes="info-row")
                )

            # Orphaned components
            container.mount(Label("", classes="info-row"))  # spacer
            container.mount(
                Label("Orphaned Components:", classes="section-header")
            )
            if self._lineage_analysis.orphaned_components:
                ...
                for comp_id in self._lineage_analysis.orphaned_components:
                    ...
                    if comp:
                        container.mount(
                            Label(f"  {comp.display_name}", classes="orphaned")
                        )
            else:
                container.mount(
                    Label("  No orphaned components", classes="info-row")
                )

            # Pipeline statistics
            container.mount(Label("", classes="info-row"))  # spacer
            container.mount(
                Label("Pipeline Statistics:", classes="section-header")
            )
            sources = sum(
                1
                for c in self._lineage_analysis.components
            ...
            container.mount(
                Label(
                    f"  Total components: {len(self._lineage_analysis.components)}",
                    classes="info-row",
                )
            )
            container.mount(
                Label(f"  Data sources: {sources}", classes="info-row")
            )
            ...
            # Complexity metrics
            total_deps = sum(
                len(c.dependencies) for c in self._lineage_analysis.components
            )
            avg_deps = (
                total_deps / len(self._lineage_analysis.components)
                if self._lineage_analysis.components
                else 0
            )

            container.mount(
                Label(
                    f"  Total dependencies: {total_deps}", classes="info-row"
                )
            )
            container.mount(
                Label(
                    f"  Average dependencies per component: {avg_deps:.1f}",
                    classes="info-row",
                )
            )
...
class QueryLineageScreen(VimNavigableScreen[PipelineComponent]):
    ...
    def __init__(self, config_manager: ConfigManager, **kwargs) -> None:
        ...
        self._filter_type: str | None = (
            None  # "source", "query", "output", or None for all
        )
    ...
    def footer_hints(self) -> str:
        filter_hint = (
            f" [{self._filter_type}]" if self._filter_type else " [all]"
        )
    ...
    def _mount_pipeline_summary(self) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    "_mount_pipeline_summary: #screen-footer not found"
                )
