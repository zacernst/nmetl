...
class EntityEditorScreen(VimEditableScreen[EntitySourceViewModel]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._adapter = adapter or DataModelAdapter(
    ...
    def update_detail_panel(self, item: EntitySourceViewModel | None) -> None:
        ...
        try:
            ...
            if item is None:
                panel.mount(
                    Label("(no source selected)", classes="detail-title")
                )
                return

            panel.mount(
                Label(f"Source: {item.source_id}", classes="detail-title")
            )
            panel.mount(Label(f"  URI: {item.uri}", classes="detail-row"))
            panel.mount(
                Label(
                    f"  Entity Type: {item.entity_type}", classes="detail-row"
                )
            )
            if item.id_col:
                panel.mount(
                    Label(f"  ID Column: {item.id_col}", classes="detail-row")
                )
            if item.query:
                panel.mount(
                    Label(
                        f"  Query: {item.query[:50]}...", classes="detail-row"
                    )
                )
            if item.schema_hints:
                ...
                for col, dtype in item.schema_hints:
                    panel.mount(
                        Label(f"    {col}: {dtype}", classes="detail-row")
                    )

            panel.mount(Label("", classes="detail-row"))
            panel.mount(
                Label("Press Enter to edit fields", classes="detail-label")
            )
    ...
    def get_fields(
        self, item: EntitySourceViewModel | None
    ) -> list[EditableField]:
        if item is None:
            return [
                EditableField(
