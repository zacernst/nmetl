...
_FUNCTION_PATTERN = re.compile(
    r"\b(" + "|".join(re.escape(f) for f in CYPHER_FUNCTIONS) + r")\s*\(",
    re.IGNORECASE,
)
...
def _tokenize_line_cached(line: str) -> tuple[SyntaxToken, ...]:
    ...
    for m in _FUNCTION_PATTERN.finditer(line):
        ...
        if not _overlaps_any(m.start(), fn_end, tokens):
            tokens.append(
                SyntaxToken(
                    m.start(), fn_end, "function", m.group().rstrip("(")
                )
            )
    ...
    for m in _KEYWORD_PATTERN.finditer(line):
        if not _overlaps_any(m.start(), m.end(), tokens):
            tokens.append(
                SyntaxToken(m.start(), m.end(), "keyword", m.group())
            )
...
def get_completions(
    prefix: str, entity_types: list[str] | None = None
) -> list[str]:
    ...
    results.extend(
        fn for fn in CYPHER_FUNCTIONS if fn.lower().startswith(prefix.lower())
    )

    # Entity types from config
    if entity_types:
        results.extend(
            et for et in entity_types if et.lower().startswith(prefix.lower())
        )
...
class CypherEditor(Widget):
    ...
    def _clamp_cursor(self) -> None:
        ...
        self.cursor.line = max(
            0, min(self.cursor.line, self.buffer.line_count - 1)
        )
        ...
        self.cursor.col = max(
            ...
            min(
                self.cursor.col,
                max(0, line_len - 1) if self.mode == "normal" else line_len,
            ),
        )
    ...
    def handle_key_normal(self, key: str) -> bool:
        ...
        match key:
            ...
            case "j" | "down":
                self.cursor.line = min(
                    self.cursor.line + 1, self.buffer.line_count - 1
                )
                self._clamp_cursor()
            case "k" | "up":
            ...
            case "l" | "right":
                ...
                self.cursor.col = min(
                    self.cursor.col + 1, max(0, line_len - 1)
                )
            # Word movement
            case "w":
            ...
            case "a":
                self.cursor.col = min(
                    self.cursor.col + 1, len(self.current_line)
                )
                self.mode = "insert"
            case "o":
                self._save_undo()
            ...
            case "p":
                if self._yanked_line is not None:
                    ...
                    self.buffer.lines.insert(
                        self.cursor.line + 1, self._yanked_line
                    )
                    self.cursor.line += 1
                    self.cursor.col = 0
            case "x":
    ...
    def search(self, pattern: str) -> None:
        ...
        if self._search_matches:
            ...
            for i, (line, col) in enumerate(self._search_matches):
                if line > self.cursor.line or (
                    line == self.cursor.line and col >= self.cursor.col
                ):
    ...
    def _move_word_forward(self) -> None:
        ...
        while col < len(line) and (line[col].isalnum() or line[col] == "_"):
            col += 1
        # Skip whitespace
        while col < len(line) and not (
            line[col].isalnum() or line[col] == "_"
        ):
            col += 1
    ...
    def _move_word_backward(self) -> None:
        ...
        if col == 0 and self.cursor.col == 0 and self.cursor.line > 0:
            self.cursor.line -= 1
            self.cursor.col = max(
                0, len(self.buffer.get_line(self.cursor.line)) - 1
            )
        else:
            self.cursor.col = col
