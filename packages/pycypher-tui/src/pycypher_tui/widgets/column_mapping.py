...
class ColumnMapping:
    ...
    validation_status: str = (
        "unknown"  # "valid", "warning", "error", "unknown"
    )
...
class ColumnMappingWidget(Static):
    ...
    class MappingChanged(Message):
        ...
        def __init__(
            self, source_id: str, new_mapping: dict[str, str]
        ) -> None:
            super().__init__()
    ...
    def update_relationship_sources(
        self, relationship_sources: list[Any]
    ) -> None:
    ...
    def _show_loading(self) -> None:
        ...
        try:
            ...
            container.mount(
                Label("Column Mapping Validation", classes="mapping-header")
            )
    ...
    def _show_empty_state(self) -> None:
        ...
        try:
            ...
            container.mount(
                Label(
                    "No relationship sources to validate.", classes="help-text"
                )
            )
        except Exception as exc:
    ...
    def _validate_column_mappings(self) -> MappingValidationResult:
        ...
        for source in self._relationship_sources:
            try:
                ...
                mapping.validation_status, mapping.validation_message = (
                    self._validate_mapping(mapping, source_columns)
                )
                ...
                logger.warning(
                    "Failed to validate source %s: %s", source.id, exc
                )
                result.issues.append(
                    {
                        "type": "error",
    ...
    def _validate_mapping(
        self, mapping: ColumnMapping, source_columns: dict[str, str]
    ) -> tuple[str, str]:
        ...
        if mapping.source_col not in source_columns:
            return (
                "error",
                f"Source column '{mapping.source_col}' not found in data",
            )
        ...
        if mapping.target_col not in source_columns:
            return (
                "error",
                f"Target column '{mapping.target_col}' not found in data",
            )
    ...
    def _update_display(self) -> None:
        ...
        try:
            ...
            if self._validation_result.mappings:
                ...
                container.mount(
                    Label("Column Mappings:", classes="mapping-header")
                )

                for i, mapping in enumerate(self._validation_result.mappings):
                    self._render_mapping_row(
                        container, mapping, i == self.selected_index
                    )
            ...
            if self._validation_result.issues:
                container.mount(Label("", classes="help-text"))  # spacer
                container.mount(
                    Label("Validation Issues:", classes="mapping-header")
                )
    ...
    def _render_mapping_row(
        self, container: Container, mapping: ColumnMapping, selected: bool
    ) -> None:
        ...
        with Horizontal():
            ...
            arrow = (
                "━━━━━>" if mapping.validation_status == "valid" else "━━━━━✗"
            )
    ...
    def _enter_edit_mode(self) -> None:
        ...
        selected_mapping = self._validation_result.mappings[
            self.selected_index
        ]
    ...
    def _show_mapping_editor(self, mapping: ColumnMapping) -> None:
        ...
        try:
            ...
            available_columns = list(
                self._validation_result.source_schema.keys()
            )
            ...
            with Horizontal():
                editor_container.mount(
                    Label("Source Column:", classes="info-label")
                )
            ...
            with Horizontal():
                editor_container.mount(
                    Label("Target Column:", classes="info-label")
                )
    ...
    def _save_mapping_changes(self) -> None:
        ...
        try:
            ...
            # Update the mapping
            if self._validation_result and 0 <= self.selected_index < len(
                self._validation_result.mappings
            ):
                ...
                try:
                    ...
                    if focused and focused.id == "source-column-select":
                        target_select = self.query_one(
                            "#target-column-select", Select
                        )
                        target_select.focus()
                    elif focused and focused.id == "target-column-select":
                        source_select = self.query_one(
                            "#source-column-select", Select
                        )
                        source_select.focus()
                except Exception:
                    pass
