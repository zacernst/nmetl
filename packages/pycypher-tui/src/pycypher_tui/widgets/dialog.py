...
class VimDialog(ModalScreen[DialogResponse]):
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "btn-ok":
            self.dismiss(DialogResponse(DialogResult.CONFIRMED))
...
class ConfirmDialog(VimDialog):
    ...
    def compose_buttons(self) -> ComposeResult:
        with Horizontal(id="dialog-buttons"):
            yield Button("[y]es", id="btn-ok", variant="primary")
...
class InputDialog(VimDialog):
    ...
    def on_input_submitted(self, event: Input.Submitted) -> None:
        self.dismiss(DialogResponse(DialogResult.CONFIRMED, value=event.value))
