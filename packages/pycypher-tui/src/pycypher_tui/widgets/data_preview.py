...
class DataPreviewDialog(VimDialog):
    ...
    def on_mount(self) -> None:
        ...
        self._worker = self.run_worker(
            self._load_preview_data, thread=True, exclusive=True
        )
    ...
    def _load_preview_data(self) -> None:
        ...
        try:
            ...
            sample_table = sampler.sample(
                n=100, strategy=SamplingStrategy.HEAD
            )
            ...
            logger.exception(
                "Failed to load preview data for %s", self.source_uri
            )
        ...
        try:
            ...
            if self._preview_data.sample_data:
                ...
                for row in self._preview_data.sample_data:
                    table.add_row(
                        *[str(row.get(col, "")) for col in first_row.keys()]
                    )
    ...
    def _update_schema_tab(self) -> None:
        ...
        try:
            ...
            schema_container.mount(
                Label(
                    f"Total Rows: {schema['row_count']:,}", classes="info-row"
                )
            )
            schema_container.mount(
                Label(
                    f"Total Columns: {len(schema['columns'])}",
                    classes="info-row",
                )
            )
            ...
            for col_name, col_type in schema["columns"]:
                schema_container.mount(
                    Label(f"  {col_name}: {col_type}", classes="info-row")
                )
        ...
        try:
            ...
            for col_name, stats in self._preview_data.column_stats.items():
                stats_container.mount(
                    Label(f"{col_name}:", classes="stat-header")
                )
                stats_container.mount(
                    Label(f"  Type: {stats.dtype}", classes="info-row")
                )
                stats_container.mount(
                    Label(
                ...
                if stats.min_value is not None:
                    stats_container.mount(
                        Label(f"  Min: {stats.min_value}", classes="info-row")
                    )
                if stats.max_value is not None:
                    stats_container.mount(
                        Label(f"  Max: {stats.max_value}", classes="info-row")
                    )
