...
class DataModelAdapter:
    ...
    def entity_detail(self, entity_type: str) -> EntityDetailViewModel:
        ...
        config = self._config_manager.get_config()
        sources = [
            s for s in config.sources.entities if s.entity_type == entity_type
        ]
    ...
    def relationship_types(self) -> list[RelationshipViewModel]:
        ...
        for rel_type in sorted(by_type):
            ...
            status = self._compute_relationship_status(
                rel_type, sources, entity_by_id_col
            )

            result.append(
                RelationshipViewModel(
                    relationship_type=rel_type,
                    source_entity=mappings[0].source_entity
                    if mappings
                    else None,
                    target_entity=mappings[0].target_entity
                    if mappings
                    else None,
                    source_count=len(sources),
    ...
    def relationship_detail(
        ...
        issues = self._validate_relationship(
            rel_type, sources, entity_by_id_col
        )
    ...
    def model_statistics(self) -> ModelStatsViewModel:
        ...
        total_sources = len(config.sources.entities) + len(
    ...
    def _validate_entity(
        self, entity_type: str, sources: list
