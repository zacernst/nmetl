...
class QueryPlanAnalyzer:
    ...
    def _build_table_stats(self) -> None:
        ...
        When the backend has a DuckDB table registry, each table's relation
        is handed to :class:`TableStatistics` so column statistics are
        computed in SQL instead of by materialising the source into pandas
        (see ``docs/duckdb_eager_path_design.md``, Phase 2).  Construction
        stays cheap either way — statistics are computed lazily, per column,
        on first access.
        """
        ...
        registry = getattr(
            getattr(self.context, "backend", None), "tables", None
        )

        def _relation(name: str, kind: str) -> Any:
            if registry is None:
                return None
            try:
                return registry.relation(name, kind)
            except Exception:  # noqa: BLE001 — stats are best-effort; pandas path still works
                return None

        for name, et in self.context.entity_mapping.mapping.items():
            self._table_stats[name] = TableStatistics(
                et.source_obj,
                relation=_relation(name, ENTITY_KIND),
            )
        for name, rt in self.context.relationship_mapping.mapping.items():
            self._table_stats[name] = TableStatistics(
                rt.source_obj,
                relation=_relation(name, RELATIONSHIP_KIND),
            )
    ...
    def log_cardinality_feedback(
        ...
        for i, (est, act) in enumerate(
            ...
            ratio = act / max(est, 1)
            log_level = (
                logging.DEBUG if 0.5 <= ratio <= 2.0 else logging.WARNING
            )
            LOGGER.log(
                log_level,
