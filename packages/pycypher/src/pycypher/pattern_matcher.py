...
class PatternMatcher:
    ...
    def _traverse_fixed_hop(
        ...
        pushdown_ids: Any = None
        # var_names / len(frame) rather than .bindings — reading the driving
        # frame's columns or row count here would materialise it, and a lazy
        # frame can supply its ids as a relation instead of a Series.
        if prev_var in frame.var_names and len(frame) > 0:
            if frame.is_lazy:
                from pycypher.scan_operators import LazyIds

                pushdown_ids = LazyIds(frame.relation, prev_var)
            else:
