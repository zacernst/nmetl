...
class InstrumentedBackend:
    ...
    @property
    def tables(self) -> Any:
        """Proxy the inner backend's table registry, if any.

        Same rationale as :attr:`connection` — the registry is reached
        directly rather than through the ``BackendEngine`` protocol, so it
        has to be forwarded explicitly.  Without this, wrapping a DuckDB
        backend for instrumentation (``nmetl run -v``) would hide the
        registry and silently disable every consumer of it.
        """
