    ...
    def read(self) -> pa.Table:
        ...
        with duckdb.connect() as con:
            ...
            if self._schema_hints:
                ...
                raw_columns = {
                    row[0]
                    for row in con.execute("DESCRIBE __raw_source").fetchall()
                }
                applicable = {
                    col: t
    ...
    *,
    ...
    if path_lower.endswith(".csv"):
        return FileDataSource(
            uri, CsvFormat(), query=query, schema_hints=schema_hints
        )
    if path_lower.endswith(".parquet"):
        return FileDataSource(
            uri, ParquetFormat(), query=query, schema_hints=schema_hints
        )
    if path_lower.endswith(".json"):
        return FileDataSource(
            uri, JsonFormat(), query=query, schema_hints=schema_hints
        )
