...
#: Scratch column carrying original row position through the endpoint
#: grouping.  Lives only in a temporary two-key table built inside
#: :func:`_dedup_endpoints`, so it cannot collide with a caller's column.
...
def _dedup_endpoints(table: pa.Table) -> pa.Table:
    ...
    # Hash-group on the endpoint pair and keep each group's smallest row
    # index.  Arrow does this natively, so the keys never leave columnar
    ...
    # irrelevant: an Arrow column has one type, so the composite key never
    # becomes an object array in the first place.
    #
    # Null endpoints group together rather than each forming their own
    # group.  That matches ``duplicated()``, which treats NaN as equal to
    # NaN, so a table with several null-to-null rows still collapses to one.
    row_index = pa.array(range(n_before), type=pa.int64())
    keys = pa.table(
        {
            "__SOURCE__": table.column("__SOURCE__"),
            "__TARGET__": table.column("__TARGET__"),
            _ROW_INDEX: row_index,
        },
    )
    first = keys.group_by(["__SOURCE__", "__TARGET__"]).aggregate(
        [(_ROW_INDEX, "min")],
    )
    if first.num_rows == n_before:
        return table

    # Grouping does not preserve input order, so sort the surviving indices
    # before taking.  Without this the output would be permuted, which is a
    # silent change: the caller assigns sequential ``__ID__``s straight
    # afterwards, so a reordering renumbers every edge.
    keep_indices = first.column(f"{_ROW_INDEX}_min")
    keep_indices = keep_indices.take(pc.sort_indices(keep_indices))
    table = table.take(keep_indices)
    n_after = table.num_rows
    LOGGER.warning(
