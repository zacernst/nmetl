...
class Context(BaseModel):
    ...
    def __init__(
        self, *, backend: Any = None, instrument: bool = False, **data: Any
    ) -> None:
    ...
    def index_manager(self) -> Any:
        ...
        if self._index_manager is None:
            ...
            # Prebuilding every store copies each entity *and* relationship
            # table into object-dtype numpy arrays whether or not the query
            # touches them.  On DuckDB that is an unbounded RAM cost paid up
            # front for tables a query may never read, so the prebuild is
            # skipped there and stores are built on demand by
            # ``get_vectorized_store()`` instead — same results, only for the
            # types actually used.  See docs/duckdb_eager_path_design.md,
            # Phase 6.
...
class EntityTable(Relation):
    ...
    def model_post_init(self, __context: Any) -> None:
        super().model_post_init(__context)
        try:
            n_rows = len(self.source_obj) if self.source_obj is not None else 0
        except TypeError:
            n_rows = "?"
        LOGGER.debug(
