...
#: column is stashed in its resolve() attribute map, so ``id()``/
#: ``elementId()`` can reuse the same ``resolve(var, prop)`` lookup as
#: ordinary property access instead of needing a second callable threaded
#: through every call site. A NUL byte can't appear in a Cypher property
#: name, so this can never collide with a real ``schema_hints`` entry.
ID_SENTINEL = "\x00id\x00"

#: Cypher function names that resolve to a node/relationship's internal ID
#: (``elementId`` is the Neo4j 5.x alias for ``id``) rather than a UDF call.
_ID_FUNCS = frozenset({"id", "elementid"})
...
def compile_expression(
    ...
    def rec(node: Any) -> str | None:
        ...
        # --- id(var) / elementId(var): the variable's physical ID column ---
        if (
            isinstance(node, FunctionInvocation)
            and node.name.lower() in _ID_FUNCS
        ):
            raw_args = (
        ...
        if isinstance(node, CaseExpression):
            if not node.when_clauses:
                return None
            simple_expr_sql = None
            if node.expression is not None:
                simple_expr_sql = rec(node.expression)
                if simple_expr_sql is None:
                    return None
            when_parts = []
            for wc in node.when_clauses:
                cond_sql = rec(wc.condition)
                result_sql = rec(wc.result)
                if cond_sql is None or result_sql is None:
                    return None
                when_parts.append(f"WHEN {cond_sql} THEN {result_sql}")
            else_sql = ""
            if node.else_expr is not None:
                compiled_else = rec(node.else_expr)
                if compiled_else is None:
                    return None
                else_sql = f" ELSE {compiled_else}"
            case_head = (
                f"CASE {simple_expr_sql}"
                if simple_expr_sql is not None
                else "CASE"
            )
            return f"({case_head} {' '.join(when_parts)}{else_sql} END)"
