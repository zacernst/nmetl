...
def source_to_pandas(obj: Any) -> pd.DataFrame:
    ...
    # A streaming entity's source is a LazyEntitySource proxy rather than a
    # frame; it converts the same way.  Duck-typed to keep this module free
    # of pycypher imports (see the module docstring).
    if not isinstance(obj, pd.DataFrame) and hasattr(obj, "to_pandas"):
        return obj.to_pandas()
    return obj
