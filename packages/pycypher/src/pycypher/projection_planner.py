...
class ProjectionPlanner:
    ...
    def _projected_sort_column(
        self,
        expression: Any,
        clause: Any,
        df: pd.DataFrame,
    ) -> str | None:
        """Return the projected column that already computes *expression*.

        In Cypher, an ``ORDER BY`` key that also appears in the projection
        refers to the *projected* value — ``RETURN p.city, count(p) ORDER BY
        p.city`` sorts the grouped rows by their grouping key.  Resolving it
        to the output column is therefore not merely an optimisation: after
        an aggregation or ``DISTINCT`` the pre-projection frame has a
        different number of rows, so re-evaluating the key there produces
        values that cannot be aligned with the result at all.

        Returns ``None`` when the expression is not part of the projection,
        leaving the caller's existing evaluation path to handle it.
        """

        def _first_present(*names: str | None) -> str | None:
            for name in names:
                if name and name in df.columns:
                    return name
            return None

        for item in getattr(clause, "items", None) or []:
            item_expression = getattr(item, "expression", None)
            if item_expression is None:
                continue
            # Structural AST equality, not rendered text: the renderer emits
            # a PropertyLookup as the bare property name, so `p.name` and
            # `q.name` render identically and a text comparison silently
            # matches the wrong projection item.
            if item_expression != expression:
                continue
            column = _first_present(
                getattr(item, "alias", None),
                self.infer_alias(item_expression),
                self.qualify_alias(item_expression),
            )
            if column is not None:
                return column

        # The expression may not appear verbatim in the projection but still
        # name one of its output columns (e.g. ORDER BY an alias).
        return _first_present(
            self.infer_alias(expression),
            self.qualify_alias(expression),
        )
    ...
    def apply_projection_modifiers(
        ...
        if order_by:
            ...
            for idx, order_item in enumerate(order_by):
                ...
                # Prefer the projected column computing this expression.  It
                # is both the Cypher-correct referent and the only source
                # with the right number of rows once a projection has
                # grouped or de-duplicated them.
                projected = self._projected_sort_column(
                    order_item.expression, clause, df
                )
                if projected is not None:
                    sort_series = temp_df[projected].reset_index(drop=True)
                else:
                    try:
                        ...
                        # VariableNotFoundError (ValueError subclass) is
                        # expected when the ORDER BY expression references a
                        # variable that was not carried into the projected
                        # frame.  Fall back to the full pre-projection frame
                        # which still has all bindings.
                        LOGGER.debug(
                ...
                if len(sort_series) != len(temp_df):
                    # Nothing sensible to sort by: the key was evaluated
                    # against a frame of a different height than the result,
                    # so pairing the two is arbitrary.  Zipping them anyway
                    # (as this did before) produced a plausible-looking but
                    # wrongly-ordered result, which is worse than saying so.
                    # Cypher itself rejects this shape: a sort key may not
                    # reach past a DISTINCT or an aggregation.
                    msg = (
                        f"ORDER BY key {idx} produced {len(sort_series)} "
                        f"values for {len(temp_df)} result rows. The sort "
                        "expression must be part of the projection when the "
                        "projection changes the number of rows (aggregation "
                        "or DISTINCT) — alias it in RETURN/WITH and order by "
                        "that alias."
                    )
                    raise ValueError(msg)
                temp_df[sort_col] = sort_series.values
