...
class ClauseExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        self._dispatch_table: dict[
            type, Callable[[Any, Any, int | None], Any]
        ] = {
    ...
    def frame_size(frame: Any) -> str:
        ...
        Uses ``len(frame)`` rather than ``len(frame.bindings)``: this feeds a
        DEBUG log message, whose arguments are evaluated eagerly, so reading
        ``.bindings`` here would materialise a lazy frame after every clause
        purely to format a string.  ``BindingFrame.__len__`` answers from
        ``COUNT(*)`` while the frame is still lazy.
        """
        if frame is None:
            return "(none)"
        try:
    ...
    def execute_query_inner(
        ...
        for clause_idx, clause in enumerate(query.clauses):
            ...
            _clause_memory[_clause_name] = _clause_memory.get(
                _clause_name, 0.0
            ...
            # isinstance, not hasattr: `bindings` is a materialise-on-access
            # property, so hasattr() would execute the relation just to
            # confirm the attribute exists.
            if _live is not None and isinstance(result, BindingFrame):
                ...
                if _dead_cols:
                    ...
                    result = result.drop_columns(_dead_cols)
