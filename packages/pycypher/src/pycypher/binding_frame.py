...
def _cleanup_merged_lazy(
    merged: Any,
    left_col: str | None,
    right_col: str | None,
) -> Any:
    """Lazy counterpart of :meth:`BindingFrame._cleanup_merged`.

    Drops the redundant right join key and any ``_right`` collision suffixes
    by projection.  Returns *merged* untouched when there is nothing to drop,
    so a join that needs no cleanup adds no extra relation node.
    """
    ...
    if (
        ...
        and right_col in columns
    ):
        drop.add(right_col)
    drop.update(c for c in columns if c.endswith("_right"))
    if not drop:
        return merged
...
def _is_lazy_frame(obj: Any) -> bool:
    """Return ``True`` if *obj* is a ``DuckDBLazyFrame``.

    Imported lazily: this is on the ``BindingFrame`` construction path, and
    the backend module is not needed at all on a pure-pandas run.
    """
    from pycypher.backends.duckdb_backend import DuckDBLazyFrame
...
def concat_binding_frames(
    ...
    *,
    ...
    """Concatenate *frames* into one, staying lazy when the backend can.

    Each frame contributes its relation when it is still lazy and its
    DataFrame otherwise, so a set of lazy scans concatenates as a DuckDB
    ``UNION ALL`` without any of them materialising.  Mixed or incompatible
    inputs fall back to ``pd.concat`` inside the backend.

    Args:
        context: The query context (supplies the backend).
        frames: Frames to concatenate; must be non-empty.
        type_registry: Type registry for the combined frame.
        distinct: Whether to de-duplicate the result.

    Returns:
        The combined :class:`BindingFrame`.

    """
    backend = getattr(context, "backend", None)
    if backend is not None:
        sources = [f.relation if f.is_lazy else f.bindings for f in frames]
        combined = backend.concat(sources, ignore_index=True)
        if distinct:
            combined = backend.distinct(combined)
    else:
        combined = pd.concat(
            [f.bindings for f in frames],
            ignore_index=True,
        )
        if distinct:
...
class BindingFrame:
    ...
    A frame may instead be constructed over an unmaterialised DuckDB
    relation (``BindingFrame(relation=..., ...)``, Phase 1b of
    ``docs/duckdb_eager_path_design.md``).  :attr:`bindings` then
    materialises on first access and the frame is pandas-backed from then
    on, so every existing caller works unchanged — it simply pays the
    materialisation the moment it touches the data.

    Operators that know how to stay in SQL check :attr:`is_lazy` and consume
    :attr:`relation` *before* anything touches :attr:`bindings`.  The state
    machine is deliberately one-way: a frame is lazy until someone forces
    it, then it is pandas forever and :attr:`relation` reports ``None``.
    Serving a relation alongside an already-materialised (and possibly
    since-mutated) DataFrame would be a correctness trap, so it is not
    offered.

    Attributes:
        bindings: DataFrame whose columns are variable names (strings) and whose
            values are entity or relationship IDs.  Materialises a lazy frame
            on first access.
    ...
    def __init__(
        self,
        bindings: FrameDataFrame | None = None,
        type_registry: dict[str, str] | None = None,
        context: Any = None,
        *,
        relation: Any = None,
        _property_cache: dict[tuple[str, str], FrameSeries] | None = None,
    ) -> None:
        """Create a frame over either a pandas DataFrame or a lazy relation.

        Args:
            bindings: The materialised bindings.  Mutually exclusive with
                *relation*.
            type_registry: Variable name → entity/relationship type.
            context: The query ``Context``.
            relation: A
                :class:`~pycypher.backends.duckdb_backend.DuckDBLazyFrame`
                to bind lazily instead of *bindings*.
            _property_cache: Pre-seeded property cache; internal.

        Raises:
            ValueError: If neither or both of *bindings* and *relation* are
                given.

        """
        # A backend operation that stayed lazy returns a DuckDBLazyFrame,
        # and callers pass whatever they got straight through as bindings=.
        # Route it to relation= here rather than at 36 call sites, so a
        # newly-lazy operation can never silently store a relation where a
        # DataFrame is expected.
        if relation is None and _is_lazy_frame(bindings):
            bindings, relation = None, bindings
        if (bindings is None) == (relation is None):
            msg = (
                "BindingFrame requires exactly one of bindings= or "
                "relation=; got "
                f"{'both' if bindings is not None else 'neither'}."
            )
            raise ValueError(msg)
        self._bindings: FrameDataFrame | None = bindings
        self._relation: Any = relation
        self.type_registry = type_registry
        self.context = context
        self._property_cache: dict[tuple[str, str], FrameSeries] = (
            {} if _property_cache is None else _property_cache
        )

    # ------------------------------------------------------------------
    # Bindings access — the materialisation boundary
    # ------------------------------------------------------------------

    @property
    def bindings(self) -> FrameDataFrame:
        """The materialised bindings, forcing a lazy relation if needed."""
        if self._bindings is None:
            self._bindings = self._relation.to_pandas()
            # One-way: once materialised, the relation is released so no
            # caller can read a relation that a later pandas-side edit has
            # made stale.
            self._relation = None
        return self._bindings

    @bindings.setter
    def bindings(self, value: FrameDataFrame) -> None:
        self._bindings = value
        self._relation = None

    @property
    def is_lazy(self) -> bool:
        """Whether this frame still holds an unmaterialised relation."""
        return self._relation is not None

    @property
    def relation(self) -> Any:
        """The backing lazy relation, or ``None`` once materialised."""
    ...
    def __len__(self) -> int:
        ...
        Answered by ``COUNT(*)`` for a lazy frame, so a row count never
    ...
    def __repr__(self) -> str:
        """Summarise the frame without materialising a lazy relation."""
        state = "lazy" if self.is_lazy else f"rows={len(self)}"
        return f"BindingFrame({state}, vars={self.var_names!r})"

    @property
    def var_names(self) -> list[str]:
        """Ordered list of variable names in this frame.

        Read from the relation schema for a lazy frame, so listing variables
    ...
    def drop_columns(self, columns: list[str]) -> BindingFrame:
        """Return a new frame without *columns*, staying lazy when possible.

        Used for dead-column elimination between clauses.  A lazy frame is
        narrowed by projection, so dropping columns never forces
        materialisation — the point of eliminating them is to carry less
        data, and materialising to do it would defeat that.

        Args:
            columns: Column names to remove; names not present are ignored.

        Returns:
            A new :class:`BindingFrame` with the remaining columns and a
            type registry filtered to match.

        """
        keep = [c for c in self.var_names if c not in set(columns)]
        registry = {
            k: v for k, v in self.type_registry.items() if k in set(keep)
        }
        if self.is_lazy:
            from pycypher.backends.duckdb_backend import DuckDBLazyFrame
            from pycypher.scan_operators import _quote

            try:
                projected = self.relation.relation.project(
                    ", ".join(_quote(c) for c in keep),
                )
            except Exception:  # noqa: BLE001 — fall back to the pandas drop
                LOGGER.debug(
                    "Lazy column drop failed; materialising", exc_info=True
                )
            else:
                return BindingFrame(
                    relation=DuckDBLazyFrame(
                        projected, self.context.backend.connection
                    ),
                    type_registry=registry,
                    context=self.context,
                )
        return BindingFrame(
            bindings=self.bindings[keep],
            type_registry=registry,
            context=self.context,
        )

    def _get_property_via_id_join(
        self,
        entity_type: str,
        var_name: str,
        prop_names: list[str],
    ) -> dict[str, FrameSeries] | None:
        """Resolve properties for an already-materialised frame in DuckDB.

        For a streaming entity there is no in-memory source table, and the
        usual routes — ``VectorizedPropertyStore`` and
        ``_get_indexed_dataframe`` — would each pull the *entire* source into
        pandas to answer lookups for the ids this frame holds.  That would
        undo the streaming registration outright.

        So the ids go the other way instead: they are handed to DuckDB and
        LEFT-joined against the registered table, and only the matching
        property values come back.  Cost is bounded by the frame, not by the
        source.

        A row-order column is carried through the join and used to sort the
        result, because a join does not preserve the left side's order and
        the returned Series must stay aligned with ``self.bindings``.

        Returns ``None`` if the DuckDB route does not apply.
        """
        import pandas as _pd

        from pycypher.backends.duckdb_backend import DuckDBLazyFrame
        from pycypher.scan_operators import _entity_relation_entry, _quote

        entry = _entity_relation_entry(self.context, entity_type)
        if entry is None or ID_COLUMN not in entry.columns:
            return None
        connection = getattr(
            getattr(self.context, "backend", None), "connection", None
        )
        if connection is None:
            return None

        selected: list[tuple[str, str, str]] = []
        for index, prop in enumerate(prop_names):
            column = entry.attr_map.get(prop, prop)
            if column not in entry.columns:
                return None
            selected.append((prop, column, f"_pyc_j{index}"))

        order_alias = "_pyc_order"
        id_alias = "_pyc_jid"
        side_id = "_pyc_side_id"
        try:
            ids = self.bindings[var_name]
            probe = _pd.DataFrame(
                {id_alias: ids.to_numpy(), order_alias: range(len(ids))},
            )
            side = entry.relation.project(
                ", ".join(
                    [
                        f"{_quote(ID_COLUMN)} AS {_quote(side_id)}",
                        *(
                            f"{_quote(column)} AS {_quote(alias)}"
                            for _prop, column, alias in selected
                        ),
                    ],
                ),
            ).set_alias("_pyc_idside")
            joined = (
                connection.from_df(probe)
                .set_alias("_pyc_idprobe")
                .join(
                    side,
                    f"{_quote(id_alias)} = {_quote(side_id)}",
                    how="left",
                )
                .order(_quote(order_alias))
                .project(
                    ", ".join(
                        _quote(alias) for _prop, _column, alias in selected
                    ),
                )
            )
            frame = DuckDBLazyFrame(joined, connection).to_pandas()
        except Exception:  # noqa: BLE001 — fall through to the pandas paths
            LOGGER.debug(
                "Id-join property fetch failed for %s; using pandas",
                entity_type,
                exc_info=True,
            )
            return None
        ...
        for prop, _column, alias in selected:
            series = _normalize_mapped_result(frame[alias])
    ...
    def _get_property_from_relation(
        self,
        entity_type: str,
        var_name: str,
        prop_name: str,
    ) -> FrameSeries | None:
        """Resolve *prop_name* by joining inside DuckDB, or ``None``.

        Only for a frame that is still lazy.  The property is LEFT-joined
        onto the relation and the whole thing materialised **once**, so the
        frame's bindings and the property column come out of a single
        execution.  The alternative — materialise the ids, then build a
        ``VectorizedPropertyStore`` — copies the entire entity table into
        object-dtype numpy arrays to serve lookups for just the ids this
        frame holds, which is the memory wall this effort exists to remove.

        ``None`` whenever the DuckDB route does not apply, in which case the
        caller continues to the unchanged vectorized / hash-map paths.
        """
        from pycypher.backends.duckdb_backend import DuckDBLazyFrame
        from pycypher.scan_operators import (
            _entity_relation_entry,
            _quote,
        )

        if entity_type in self._shadow or entity_type in self._shadow_rels:
            return None
        context = self.context
        ...
        value_alias = "_pyc_value"
        id_alias = "_pyc_vid"
        columns = list(self.var_names)
        try:
            side = entry.relation.project(
                f"{_quote(ID_COLUMN)} AS {_quote(id_alias)}, "
                f"{_quote(column)} AS {_quote(value_alias)}",
            ).set_alias("_pyc_propside")
            joined = (
                self.relation.relation.set_alias("_pyc_proplhs")
                .join(
                    side,
                    f"{_quote(var_name)} = {_quote(id_alias)}",
                    how="left",
                )
                .project(
                    ", ".join(
                        [*(_quote(c) for c in columns), _quote(value_alias)],
                    ),
                )
            )
            frame = DuckDBLazyFrame(
                joined, context.backend.connection
            ).to_pandas()
        except Exception:  # noqa: BLE001 — fall through to the pandas paths
            LOGGER.debug(
                "Relation property fetch failed for %s.%s; using pandas",
                entity_type,
                prop_name,
                exc_info=True,
            )
        ...
        # The single execution above produced the bindings too — adopt them
        # so the frame is materialised exactly once rather than re-executing
        # the relation on the next `.bindings` access.
        self.bindings = frame[columns]
        result = _normalize_mapped_result(frame[value_alias])
        result.index = self._bindings.index
    ...
    def get_property(self, var_name: str, prop_name: str) -> FrameSeries:
        ...
        # --- Relation fast path (still-lazy frames) ---
        # Resolving the property inside DuckDB and materialising once beats
        # materialising the ids and then building a VectorizedPropertyStore,
        ...
        # --- Streaming source: never materialise it ---
        # Both routes below read the entity's in-memory source table, which
        # for a streaming entity means loading the whole file. Push the ids
        # into DuckDB instead.
        from pycypher.ingestion.streaming_entity import (
            context_has_streaming_source,
        )

        if context_has_streaming_source(self.context, entity_type):
            joined = self._get_property_via_id_join(
                entity_type, var_name, [prop_name]
            )
            if joined is not None:
                return joined[prop_name]
    ...
    def _get_properties_batch_from_relation(
        self,
        var_name: str,
        prop_names: list[str],
    ) -> dict[str, FrameSeries] | None:
        """Resolve several properties in one DuckDB join, or ``None``.

        The batched counterpart of :meth:`_get_property_from_relation`: all
        requested columns are joined on in a single pass and the relation is
        materialised once, so a ``RETURN p.a, p.b, p.c`` costs one execution
        rather than one per property plus a whole-table property store.

        Returns ``None`` if the DuckDB route does not apply, leaving the
        caller's existing paths untouched.
        """
        from pycypher.backends.duckdb_backend import DuckDBLazyFrame
        from pycypher.scan_operators import _entity_relation_entry, _quote
        ...
        for index, prop in enumerate(prop_names):
            column = entry.attr_map.get(prop, prop)
            if column not in entry.columns:
                # A property the table does not have is null everywhere; let
                # the per-property path produce that, rather than special
                # casing it inside the join.
                return None
            selected.append((prop, column, f"_pyc_b{index}"))
        ...
        try:
            ...
            ).set_alias("_pyc_batchside")
            joined = (
                self.relation.relation.set_alias("_pyc_batchlhs")
                .join(
                    side,
                    f"{_quote(var_name)} = {_quote(id_alias)}",
                    how="left",
                ...
                .project(
                    ", ".join(
                        [
                            *(_quote(c) for c in columns),
                            *(
                                _quote(alias)
                                for _prop, _column, alias in selected
                            ),
                        ],
                    ),
                )
            ...
            frame = DuckDBLazyFrame(
                joined, self.context.backend.connection
            ).to_pandas()
        except Exception:  # noqa: BLE001 — fall through to the pandas paths
            LOGGER.debug(
                "Relation batch property fetch failed for %s; using pandas",
                entity_type,
                exc_info=True,
            )
            return None
        ...
        for prop, _column, alias in selected:
            series = _normalize_mapped_result(frame[alias])
            series.index = self._bindings.index
            series.name = prop
            results[prop] = series
            self._property_cache[var_name, prop] = series
        return results
    ...
    def get_properties_batch(
        ...
        # --- Relation fast path (still-lazy frames) ---
        # One join, one materialisation, every requested property at once —
        # instead of materialising the ids and then building a
        # VectorizedPropertyStore over the whole entity table.
        if self.is_lazy:
            batched = self._get_properties_batch_from_relation(
                var_name, uncached_props
            )
            if batched is not None:
                results_from_cache.update(batched)
                return results_from_cache

        # --- Streaming source: never materialise it (see get_property) ---
        from pycypher.ingestion.streaming_entity import (
            context_has_streaming_source,
        )

        _entity_type = self.type_registry.get(var_name)
        if _entity_type is not None and context_has_streaming_source(
            self.context, _entity_type
        ):
            joined = self._get_property_via_id_join(
                _entity_type, var_name, uncached_props
            )
            if joined is not None:
                results_from_cache.update(joined)
                return results_from_cache

        # Fall back for edge cases: missing variable, multi-type, unresolvable
        # var_names, not bindings.columns — the frame may still be lazy here
        # if the relation path above declined.
        if var_name not in self.var_names:
    ...
    def _cleanup_merged(
        ...
        """
        # A lazy relation is narrowed by projection; there is no index to
        # reset, and materialising just to drop columns would undo the join's
        # laziness at the last step.
        if _is_lazy_frame(merged):
            return _cleanup_merged_lazy(merged, left_col, right_col)
    ...
    def join(
        ...
        *,
        ...
        # len(frame), not len(frame.bindings): a row count is free for a
        # lazy frame and only feeds join-strategy planning.
        _left_len = len(self)
        _right_len = len(other)
        ...
        backend = self._backend
        # Source operands: a lazy frame contributes its relation so the join
        # can compose instead of executing.
        _self_src = self.relation if self.is_lazy else self.bindings
        _other_src = other.relation if other.is_lazy else other.bindings
        if _plan.strategy == JoinStrategy.BROADCAST and _right_len > _left_len:
            ...
            if backend is not None:
                ...
                if (
                    ...
                    # Through the backend, not pandas: `merged` may still be
                    # a lazy relation and a pandas rename would force it.
                    merged = backend.rename(merged, {right_col: left_col})
            else:
    ...
    def left_join(
        ...
        if backend is not None:
            merged: pd.DataFrame = _backend_merge(
                backend,
                self.relation if self.is_lazy else self.bindings,
                other.relation if other.is_lazy else other.bindings,
                left_col=left_col,
                right_col=right_col,
                how="left",
            )
    ...
    def cross_join(self, other: BindingFrame) -> BindingFrame:
        ...
        if _DEBUG_ENABLED:
            _t0 = time.perf_counter()
        # Row counts are free for a lazy frame, and the safety ceiling below
        # must be enforced *before* anything executes.
        _left_rows = len(self)
        _right_rows = len(other)
        ...
        if backend is not None:
            ...
            left_other = set(self.var_names)
            right_other = set(other.var_names)
            collisions = left_other & right_other
            right_df = other.relation if other.is_lazy else other.bindings
            if collisions:
    ...
    def rename(
        ...
        """
        # var_names, not bindings.columns — the latter would materialise a
        # lazy frame just to check membership.
        if old_col not in self.var_names:
            from pycypher.exceptions import VariableNotFoundError
        ...
        backend = self._backend
        source = self.relation if self.is_lazy else self.bindings
        if backend is not None:
            new_bindings = backend.rename(source, {old_col: new_col})
        else:
