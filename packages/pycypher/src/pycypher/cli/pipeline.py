...
def _warn_streaming_fallback(reason: str, *, exc_info: bool = False) -> bool:
    """Emit a visible warning that the run fell back to the eager path.

    Fires on both ``click.echo(..., err=True)`` (visible on the terminal
    regardless of log configuration) and ``LOGGER.warning`` (visible to log
    aggregation) — neither channel alone is guaranteed to be seen, and this
    fallback means the run just lost its bounded-memory guarantee (see
    docs/fastopendata_streaming_qualification_plan.md, "Phase 0").  Always
    returns ``False`` so call sites can write ``return
    _warn_streaming_fallback(...)``.
    """
    from shared.logger import LOGGER

    message = (
        f"Pipeline did not qualify for out-of-core streaming: {reason}. "
        "Falling back to in-memory pandas execution; this may exceed "
...
def _classify_streaming_query(
    ...
    when the query qualifies — either as an eligible mutation
    (*mutation_kind* set, see :func:`~pycypher.relation_engine.
    is_relation_mutation_eligible`) or as a read with at least one output
    sink (*sinks* non-empty) — otherwise *reason* is the human-readable
    explanation :func:`_warn_streaming_fallback` reports for it.

    Pulled out of :func:`_try_streaming_run` (which bails on the first
    ineligible query, since one query failing the pre-check falls the whole
    run back to pandas) so a coverage-auditing tool can classify *every*
    query in a config in one pass without reimplementing this logic —
    see docs/fastopendata_streaming_qualification_plan.md, Phase 3.
    """
    ...
    if not is_relation_eligible(ast, context):
        reason = (
            f"query '{q.id}' is not eligible for the out-of-core relation "
            "engine (unsupported clause shape, an unregistered relationship "
            "traversal, or an aggregation the mutation/read compiler "
            "doesn't support yet)"
        )
    ...
    if not sinks:
        reason = (
            f"query '{q.id}' is read-eligible for streaming but has no "
...
def _try_streaming_run(
    ...
    output.  Every ``False`` return past this point means the run degraded
    to the eager path — see :func:`_warn_streaming_fallback`, called at each
    one, for why. The one exception is the ``backend_engine`` check
    immediately below: that's "streaming was never requested," not a
    fallback, so it stays silent.
    """
    ...
    try:
        ...
        if not relation_engine_enabled(context):
            return _warn_streaming_fallback(
        ...
        try:
            ...
            # Progress lines are buffered, not echoed immediately, so a
            # later ineligible query still leaves no visible "streaming"
            # output before falling back (matches the pre-3b contract).
            for qi, q in enumerate(queries, 1):
                reason, text, mutation_kind, sinks = _classify_streaming_query(
                    q, context, pipeline_config, config_dir
                )
                if reason is not None:
                    return _warn_streaming_fallback(reason)
                if mutation_kind is not None:
                    try:
                        execute_relation_mutation(
        ...
        except Exception as exc:  # noqa: BLE001 — any pre-check problem: fall back to the robust normal path
            return _warn_streaming_fallback(
                f"unexpected error during the streaming eligibility "
                f"pre-check: {exc}",
                exc_info=True,
            )

        # All queries eligible and every mutation already executed above —
        # remaining plan entries are reads; stream each to its sink(s).
