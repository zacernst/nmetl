...
class PathExpander:
    ...
    # ------------------------------------------------------------------
    # DuckDB recursive-CTE expansion (Phase 7)
    # ------------------------------------------------------------------
    ...
    def _expand_duckdb(
        ...
        min_hops: int,
        max_hops: int | None,
        path_length_col: str | None,
        row_limit: int | None,
    ) -> BindingFrame | None:
        """Expand a variable-length path with a ``WITH RECURSIVE`` CTE.

        The walk carries only ``(start_id, tip, hop)`` and re-joins the seed
        at the end, so the recursion moves three columns instead of the seed's
        full width, and ``UNION``'s set semantics provide the per-hop
        deduplication the pandas frontier does with ``duplicated()``.

        Returns ``None``, leaving the pandas BFS to run unchanged, whenever
        the SQL route would not be an exact substitute:

        * **No registered edge table**, or a pending ``_shadow_rels``
          overlay — the walk would read stale edges.
        * **Undirected traversal** — not supported by the pandas path either,
          so there is no reference behaviour to match.
        * **A ``row_limit``** — the pandas path fills hop 1, then hop 2, and
          trims at the boundary, so *which* rows survive depends on hop order
          and on frontier order within a hop. ``LIMIT`` over the CTE would
          keep a different subset. Correctness beats the pushdown here.
        * **Duplicate ``start_var`` values in the seed** — the pandas frontier
          deduplicates on ``(start_var, tip)`` while carrying the seed's other
          columns, so a repeated start value silently drops seed rows. Joining
          the narrow walk back to the seed keeps them. Rather than replicate
          that (arguably accidental) behaviour, the SQL route declines when
          the seed's start values are not unique.
        """
        ...
        from pycypher.scan_operators import _quote, _registered_table

        context = self.context
        if row_limit is not None:
            return None
        if direction not in (_RD.LEFT, _RD.RIGHT):
            return None
        if rel_type in getattr(context, "_shadow_rels", {}):
            return None
        entry = _registered_table(context, rel_type, RELATIONSHIP_KIND)
        if entry is None:
            return None
        if any(
            col not in entry.columns
            for col in (RELATIONSHIP_SOURCE_COLUMN, RELATIONSHIP_TARGET_COLUMN)
        ):
            return None

        connection = getattr(
            getattr(context, "backend", None), "connection", None
        )
        ...
        seed_columns = list(start_frame.var_names)
        if start_var not in seed_columns:
            return None
        if end_var in seed_columns or (
            path_length_col and path_length_col in seed_columns
        ):
            # The output columns must not collide with the seed's.
            return None

        try:
            seed = (
                start_frame.relation.relation
                if start_frame.is_lazy
                else connection.from_df(start_frame.bindings)
            )
            counts = seed.aggregate(
                f"count(*) AS n, count(DISTINCT {_quote(start_var)}) AS d",
            ).fetchone()
            if counts is None or counts[0] != counts[1]:
                return None

            if direction == _RD.LEFT:
                match_col, next_col = (
                    RELATIONSHIP_TARGET_COLUMN,
                    RELATIONSHIP_SOURCE_COLUMN,
                )
            ...
            projection = [f"s.{_quote(c)}" for c in seed_columns]
            projection.append(f"w.tip AS {_quote(end_var)}")
            if path_length_col:
                projection.append(f"w.hop AS {_quote(path_length_col)}")

            sql = (  # nosec B608 — identifiers quoted; hop bounds are ints
                "WITH RECURSIVE walk(start_id, tip, hop) AS ("
                f"  SELECT {_quote(start_var)}, {_quote(start_var)}, 0"
                "   FROM _pyc_seed"
                " UNION "
                "  SELECT w.start_id, "
                ...
                f"  WHERE w.hop < {int(max_val)}"
                ")"
                f" SELECT {', '.join(projection)}"
                "  FROM walk w"
                f"  JOIN _pyc_seed s ON s.{_quote(start_var)} = w.start_id"
                f" WHERE w.hop BETWEEN {int(min_hops)} AND {int(max_val)}"
            )
            relation = seed.query("_pyc_seed", sql)
        except Exception:  # noqa: BLE001 — any SQL problem: use the pandas BFS
            LOGGER.debug(
                "DuckDB path expansion failed for [:%s*]; using pandas BFS",
                rel_type,
                exc_info=True,
            )
            return None

        type_registry = dict(start_frame.type_registry)
        if end_type:
            type_registry[end_var] = end_type
        return BindingFrame(
            relation=DuckDBLazyFrame(relation, connection),
    ...
    def expand_variable_length_path(
        ...
        if expanded is not None:
            LOGGER.debug(
                "variable-length path [:%s*%s..%s] expanded via DuckDB "
                "recursive CTE",
                rel_type,
                min_hops,
                max_hops,
