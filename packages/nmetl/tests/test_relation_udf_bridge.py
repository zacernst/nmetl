...
def _ctx() -> Context:
    people = pd.DataFrame(
        {"__ID__": [1, 2, 3], "name": ["a", "bb", "ccc"], "n": [1, 2, 3]}
    )
    ctx = Context(
    ...
    def test_bridge_is_idempotent(self) -> None:
        ...
        assert is_relation_eligible(
            ASTConverter.from_cypher(
                "MATCH (p:Person) RETURN hundredx(p.n) AS b"
            ),
            ctx,
        )

    def test_bytes_returning_udf_bridges_to_blob(self) -> None:
        # decode_serialized (fastopendata's registered function) returns
        # bytes -- previously unmappable (Any was the only annotation that
        # "worked", and Any can't map to a concrete DuckDB type either), so
        # it silently never bridged. _PY_TO_DUCKDB now maps bytes -> BLOB.
        register_user_function(_to_bytes, name="tobytes")
        ctx = _ctx()
        bridge_user_functions(ctx)
        assert is_relation_eligible(
            ASTConverter.from_cypher(
                "MATCH (p:Person) RETURN tobytes(p.name) AS b"
            ),
            ctx,
        )
