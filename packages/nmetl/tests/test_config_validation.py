"""Tests for the configuration validation module (editor support).
...
- Incremental field-level validation for real-time editor feedback
...
class TestValidationResult:
    ...
    def test_error_has_structured_fields(self):
        ...
        assert (
            err.message == "Source URI '' has an unrecognised file extension."
        )
...
class TestSyntacticValidation:
    ...
    def test_empty_source_uri(self):
        config_dict = {
            "version": "1.0",
            "sources": {
                "entities": [{"id": "x", "uri": "", "entity_type": "X"}]
            },
        }
        result = validate_config_dict(config_dict)
        assert not result.is_valid
...
class TestSemanticValidation:
    ...
    def test_output_references_nonexistent_query(self):
        config_dict = {
            "version": "1.0",
            "queries": [{"id": "q1", "inline": "MATCH (n) RETURN n"}],
            "output": [{"query_id": "nonexistent", "uri": "file:///out.csv"}],
        }
        result = validate_config_dict(config_dict)
        assert not result.is_valid
    ...
    def test_output_references_valid_query(self):
        config_dict = {
            "version": "1.0",
            "queries": [{"id": "q1", "inline": "MATCH (n) RETURN n"}],
            "output": [{"query_id": "q1", "uri": "file:///out.csv"}],
        }
        result = validate_config_dict(config_dict)
        assert result.is_valid
...
class TestIncrementalValidation:
    """Tests for field-level validation suitable for real-time editor feedback."""

    def test_validate_entity_uri_valid(self):
        result = validate_field(
            "sources.entities.uri", "file:///data/test.csv"
        )
        assert result.is_valid
    ...
    def test_validate_entity_uri_bad_extension(self):
        result = validate_field(
            "sources.entities.uri", "file:///data/test.xlsx"
        )
        assert not result.is_valid
...
class TestValidateFromConfig:
    ...
    def test_valid_config(self):
        cfg = PipelineConfig(
            ...
            output=[OutputConfig(query_id="q1", uri="file:///out.csv")],
        )
    ...
    def test_semantic_error_output_ref(self):
        cfg = PipelineConfig(
            version="1.0",
            queries=[QueryConfig(id="q1", inline="MATCH (n) RETURN n")],
            output=[OutputConfig(query_id="missing", uri="file:///out.csv")],
        )
        result = validate_config(cfg)
        assert not result.is_valid
        assert any(e.category == ErrorCategory.SEMANTIC for e in result.errors)
