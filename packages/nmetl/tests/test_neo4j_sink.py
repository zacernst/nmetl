"""Unit tests for nmetl.sinks.neo4j.
...
the whole module; run with::
    ...
    uv run pytest packages/nmetl/tests/test_neo4j_sink.py -m unit -v
"""
...
def mock_driver_ctx() -> Any:
    ...
    with patch("nmetl.sinks.neo4j.GraphDatabase") as mock_gdb:
        driver = MagicMock()
        session = MagicMock()
...
class TestNeo4jSinkContextManager:
    ...
    def test_enter_returns_sink(self) -> None:
        with patch("nmetl.sinks.neo4j.GraphDatabase"):
