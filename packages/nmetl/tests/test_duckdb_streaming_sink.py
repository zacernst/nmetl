...
from nmetl.config import OutputFormat
...
class TestFormatResolution:
    def test_explicit_format_overrides_extension(
        self, con, src_parquet, tmp_path
    ) -> None:
        out = tmp_path / "out.dat"  # unknown extension
        write_relation_to_uri(
            _relation(con, src_parquet), str(out), OutputFormat.CSV
        )
        got = pd.read_csv(out)
        assert len(got) == 3
...
class TestGuards:
    def test_cloud_uri_rejected(self, con, src_parquet) -> None:
        with pytest.raises(NotImplementedError, match="Cloud output"):
            write_relation_to_uri(
                _relation(con, src_parquet), "s3://bucket/out.parquet"
            )

    def test_non_relation_rejected(self, tmp_path) -> None:
        with pytest.raises(TypeError, match="DuckDBLazyFrame"):
            write_relation_to_uri(
                pd.DataFrame({"a": [1]}), str(tmp_path / "x.csv")
            )


class TestRoundTrip:
    def test_read_relation_then_write_relation(
        self, con, src_parquet, tmp_path
    ) -> None:
