"""Pydantic models for the nmetl pipeline YAML configuration file.
...
from pycypher.ingestion.data_sources import SQL_SCHEMES, SUPPORTED_EXTENSIONS
...
_SQL_LIST = ", ".join(f"{s}://" for s in sorted(SQL_SCHEMES))
...
def _check_source_uri(uri: str, query: str | None) -> None:
    ...
    * *uri* has neither a recognised SQL scheme nor a supported file extension.
    ...
    if not any(path_lower.endswith(ext) for ext in SUPPORTED_EXTENSIONS):
        msg = (
...
# OutputFormat lives in pycypher because Star.stream_query_to_uri writes
# results too; it is re-exported here for pipeline-config users.
...
class RelationshipSourceConfig(BaseModel):
    ...
    Attributes:
        ...
        source_entity_type: Entity label every edge in this source starts
            from. When set, the out-of-core relation engine only traverses
            these edges from nodes of that label, so two entity types that
            share id values (e.g. 1-year and 5-year survey files keyed by
            the same ``SERIALNO``) each see only their own edges. Optional;
            an edge with no declared type matches any label.
