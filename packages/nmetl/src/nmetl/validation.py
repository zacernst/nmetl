"""Structured configuration validation for editors and programmatic use.

Provides validation of pipeline configurations with structured error
objects suitable for real-time display in an editor, including:
...
from pycypher.ingestion.data_sources import SUPPORTED_EXTENSIONS
...
class ValidationError:
    ...
    Attributes:
        ...
        suggestion: Optional fix suggestion to show the user.
...
class ValidationResult:
    ...
    Used by interactive editors to display real-time validation feedback with
...
def _validate_version(value: Any) -> tuple[bool, str]:
    ...
    if not _VERSION_RE.match(value):
        return (
            False,
            f"Invalid version format: {value!r}. Expected '<major>.<minor>'.",
        )
    return True, ""
...
def validate_field(field_path: str, value: Any) -> ValidationResult:
    """Validate a single field value for real-time editor feedback.
...
def suggest_migration(config_dict: dict[str, Any]) -> str | None:
    ...
    lines = [
        ...
        "",
        "See the canonical schema in 'nmetl.config.PipelineConfig'.",
    ]
    return "\n".join(lines)
