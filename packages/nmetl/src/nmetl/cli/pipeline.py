...
imported from :mod:`nmetl.cli.common`.
"""
...
from nmetl.cli.common import (
    ErrorPolicyTracker,
    cli_error,
    get_pipeline_error_label,
    load_config,
)
from pycypher.ingestion.data_sources import SQL_SCHEMES
...
def dry_run_validate(
    ...
    *,
    ...
    for src, kind, label in all_sources:
        ...
        if scheme in SQL_SCHEMES or scheme in (
            "http",
            "https",
            "s3",
            "gs",
...
def _note_in_memory_path(reason: str) -> bool:
    """Say, visibly, that this run is using the in-memory engine and why.

    Only reached when out-of-core streaming was never requested (the
    relation engine is off). With the engine *on* there is no fallback:
    a query the engine cannot run is a per-query error (see
    :func:`_try_streaming_run`). Always returns ``False`` so call sites can
    write ``return _note_in_memory_path(...)``.
    """
    from shared.logger import LOGGER

    message = (
        f"Running on the in-memory engine: {reason}. This may exceed "
...
def _classify_streaming_query(
    q: Any,
    context: Any,
    pipeline_config: Any,
    config_dir: Path,
) -> tuple[str | None, str | None, str | None, list[Any]]:
    """Classify one query for the out-of-core streaming pre-check.

    Returns ``(reason, text, mutation_kind, sinks)``. *reason* is ``None``
    when the query translates — either as a mutation (*mutation_kind* set,
    see :func:`~pycypher.relation_engine.is_relation_mutation_eligible`) or
    as a read (*sinks* may be empty: the read is still executed for any
    side effects it carries, and its result discarded) — otherwise
    *reason* names the unsupported construct, which
    :func:`_try_streaming_run` reports through the per-query error policy.

    Shared with the coverage-auditing test so it classifies every query in
    a config with exactly this logic — see
    the FastOpenData streaming-qualification plan (private repository), Phase 3.
    """
    from pycypher.ast_converter import ASTConverter
    from pycypher.plan.report import UNSUPPORTED, classify
    from pycypher.relation_engine import is_relation_mutation_eligible
    ...
    if q.inline is not None:
        ...
        return (
            f"query '{q.id}' has neither inline text nor a source file",
            None,
            None,
            [],
        )
    ...
    status, construct = classify(ast, context)
    if status == UNSUPPORTED:
        reason = (
            f"query '{q.id}' uses a construct the out-of-core relation "
            f"engine does not support: {construct}"
        )
        return reason, text, None, []
    mutation_kind = is_relation_mutation_eligible(ast, context)
    if mutation_kind is not None:
        return None, text, mutation_kind, []
    sinks = [o for o in pipeline_config.output if o.query_id == q.id]
    return None, text, None, sinks
...
def _try_streaming_run(
    ...
    Returns ``True`` if the run went through the streaming path (caller
    should stop), or ``False`` if streaming was never requested — the
    backend is not ``duckdb`` or the relation engine is off — in which
    case the caller runs the in-memory path.

    Once streaming is requested there is no fallback (generalisation plan,
    Phase 5): a query the relation engine cannot translate is reported as
    a per-query error naming the unsupported construct, under the same
    ``--on-error`` policy as any other query failure, and a source that
    fails to register is a hard error. Mutations run as they are
    classified (a later query may read a property an earlier ``SET``
    creates); reads stream to their sinks afterwards, and a read with no
    sink is executed for its side effects and discarded.
    """
    ...
    try:
        ...
        if not relation_engine_enabled(context):
            return _note_in_memory_path(
                "backend_engine is 'duckdb' but the relation engine is not "
                "enabled (set relation_engine: true in the pipeline config, "
                "or the PYCYPHER_DUCKDB_RELATION_ENGINE environment variable)"
            )
        ...
        if verbose:
            ...
            LOGGER.info(
                "Backend: %s (out-of-core streaming)", context.backend_name
            )
        ...
        plan: list[tuple[int, Any, str, list[Any]]] = []
        ...
        try:
            ...
            for rel_src in pipeline_config.sources.relationships:
                rel_ds = data_source_from_uri(
                    rel_src.uri,
                    query=rel_src.query,
                ...
                register_streaming_relationship(
                    ...
                    source_entity_type=rel_src.source_entity_type,
                    target_entity_type=rel_src.target_entity_type,
                )

        except Exception as exc:  # noqa: BLE001 — any registration failure is fatal: there is no in-memory fallback once streaming is requested
            raise click.ClickException(
                f"Could not register a data source for streaming: {exc}"
            ) from exc

        click.echo(f"Streaming {n_queries} query/queries (out-of-core) …")
        # Mutations execute here, interleaved with classification
        # (Phase 3b — the FastOpenData streaming-qualification plan (private repository)),
        # rather than deferred to a second pass: a later query may read a
        # property this mutation's SET creates fresh, and needs it to
        # already exist when *its own* translation is checked below.
        # `queries` is already dependency-sorted by
        # `_reorder_queries_by_dependency` (property-level produces/
        # consumes, computed before this function is called), so by the
        # time a consumer is reached here its producer has already run.
        for qi, q in enumerate(queries, 1):
            reason, text, mutation_kind, sinks = _classify_streaming_query(
                q, context, pipeline_config, config_dir
            )
            if reason is not None:
                tracker.handle(reason)
                continue
            if mutation_kind is not None:
                try:
                    execute_relation_mutation(
                        ASTConverter.from_cypher(text),
                        context,
                        mutation_kind,
                    )
                    click.echo(
                        f"  [{qi}/{n_queries}] query [{q.id}] ({mutation_kind})"
                    )
                except stream_errors as exc:
                    label = get_pipeline_error_label(exc)
                    tracker.handle(f"query [{q.id}] {label}: {exc}")
                continue
            if not sinks:
                try:
                    execute_relation_query(
                    ...
                    click.echo(
                        f"  [{qi}/{n_queries}] query [{q.id}] "
                        "(no output sink; executed, result discarded)"
                    )
                except stream_errors as exc:
                    label = get_pipeline_error_label(exc)
                    tracker.handle(f"query [{q.id}] {label}: {exc}")
                continue
            plan.append((qi, q, text, sinks))

        star = Star(context=context)
        for qi, q, text, sinks in plan:
...
def run_impl(
    ...
    # run requests it (duckdb backend, relation engine enabled), stream
    # each query file -> relation -> sink via DuckDB COPY without loading
    # sources into memory. Only when streaming was never requested do we
    ...
    try:
        ...
        context = builder.build(
            backend=pipeline_config.backend_engine,
            instrument=verbose,
        )
