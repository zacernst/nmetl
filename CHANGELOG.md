...
- Repository split (2026-09-07): this repository now contains only the open-source engine (`pycypher`, `nmetl`, `shared`). The FastOpenData pipeline, API, site, and client moved to a private repository and consume these packages as dependencies.

### Removed

- The `pycypher-tui` package (VIM-style terminal UI for editing pipeline configs), its tests and docs, the `nmetl tui` sub-command, and the `fastopendata.tui` screens. The editor-facing helpers it consumed (`nmetl.pipeline_builder`, `nmetl.validation`, `nmetl.introspector`, `nmetl.data_preview`) remain as library APIs.

### Changed

#### Package split: pycypher / nmetl
- The ETL layer now lives in its own workspace package, `nmetl`, which depends on `pycypher`; `pycypher` no longer ships the `nmetl` console script or depends on `click`/`pyyaml`.
- Moved to `nmetl`: `nmetl_cli`, `cli/*`, `_cli_query`, `repl`, `health_server`, `sinks.neo4j`, and the pipeline-config half of `pycypher.ingestion` (`config` → `nmetl.config`, `validation`, `pipeline_builder`, `introspector`, `data_preview`).
- `OutputFormat` now lives in `pycypher.ingestion.output_writer` (re-exported by `nmetl.config`) because `Star.stream_query_to_uri` needs it.
- Stale imports such as `from pycypher.ingestion import PipelineConfig` raise an `AttributeError` naming the new module.
- `fastopendata` now depends on `nmetl`.
- Tests for the moved modules live in `packages/nmetl/tests`.
